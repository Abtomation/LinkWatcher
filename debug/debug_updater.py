...
def debug_updater():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for ref in references:
            print(f"   • {ref.file_path}:{ref.line_number} → '{ref.link_target}' (type: {ref.link_type})")
        ...
        if references:
            print(f"   Calling updater.update_references({len(references)} refs, '{old_path}' → '{new_path}')")
