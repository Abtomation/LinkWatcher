...
def find_linkwatcher_installation():
    ...
    for location in possible_locations:
        if (location / "link_watcher.py").exists():
            return location
...
def create_vscode_tasks(linkwatcher_dir):
        ...
        "tasks": [
            {
                ...
                "type": "shell",
                "command": "python",
                "args": [str(linkwatcher_dir / "link_watcher.py")],
                "group": "build",
                "presentation": {
                    "echo": True,
                    "reveal": "always",
    ...
    if tasks_file.exists():
        try:
            ...
            existing_config["tasks"] = [
                task for task in existing_config["tasks"] 
...
python "{linkwatcher_dir / 'link_watcher.py'}"
pause
"""
...
echo "Starting LinkWatcher for this project..."
python3 "{linkwatcher_dir / 'link_watcher.py'}"
"""
...
def main():
    ...
    if not linkwatcher_dir:
        ...
        if manual_path:
            ...
            if not (linkwatcher_dir / "link_watcher.py").exists():
    ...
    print(f"   python \"{linkwatcher_dir / 'link_watcher.py'}\"")
