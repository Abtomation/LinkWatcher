...
- [ ] **Task Definition File**: Create using [New-Task.ps1](../../scripts/file-creation/support/New-Task.ps1) and the [`task-creation` craft skill](../../../.claude/skills/task-creation/SKILL.md)

  - **Status**: [NOT_STARTED/IN_PROGRESS/COMPLETED]
  - **Dependencies**: Task concept finalized, task-creation craft skill applied
  - **Craft**: [`task-creation` skill](../../../.claude/skills/task-creation/SKILL.md)
  - **Script**: `process-framework/scripts/file-creation/support/New-Task.ps1`
  - **Command**: `pwsh.exe -ExecutionPolicy Bypass -File process-framework/scripts/file-creation/support/New-Task.ps1 -TaskName "[Task Name]" -WorkflowPhase "[phase]" -Description "[Brief description]" -Confirm:\$false`
...
- [ ] **Document Creation Script**: Create script for generating new files applying the [`creation-script-development` craft skill](../../../.claude/skills/creation-script-development/SKILL.md)

  - **Status**: [NOT_STARTED/IN_PROGRESS/COMPLETED/SKIPPED]
  - **Dependencies**: Task definition completed, directory structure created
  - **Craft**: [`creation-script-development` craft skill](../../../.claude/skills/creation-script-development/SKILL.md)
...
- [ ] **Task-Specific Template**: Create template applying the [`template-development` craft skill](../../../.claude/skills/template-development/SKILL.md) and using [New-Template.ps1](../../scripts/file-creation/support/New-Template.ps1)

  - **Status**: [NOT_STARTED/IN_PROGRESS/COMPLETED/SKIPPED]
  - **Dependencies**: Understanding of file structure needed, document creation script completed (if applicable)
  - **Craft**: [`template-development` craft skill](../../../.claude/skills/template-development/SKILL.md)
  - **Script**: `process-framework/scripts/file-creation/support/New-Template.ps1`
  - **Command**: `pwsh.exe -ExecutionPolicy Bypass -File process-framework/scripts/file-creation/support/New-Template.ps1 -TemplateName "[Template Name]" -TemplateDescription "[Description]" -DocumentPrefix "[ID-PREFIX]" -DocumentCategory "[Category]" -Confirm:\$false`
...
- [ ] **Task Usage Guide**: Create guide using New-Guide.ps1 and documentation-guide.md
  ...
  - **Command**: `pwsh.exe -ExecutionPolicy Bypass -File process-framework/scripts/file-creation/support/New-Guide.ps1 -GuideTitle "[Task Name] Usage Guide" -SubDirectory "[phase-directory]" -GuideDescription "Comprehensive guide for using the [Task Name] task effectively" -Confirm:\$false`
  - **Notes**: Explains how to use the task effectively, always needed for new tasks

### Phase 4: Documentation (Session 4)
...
- [ ] **Documentation Map Update**: Update PF-documentation-map.md with all new artifacts
  ...
  - **Artifacts to Add**: [List all new files created: task definition, scripts, templates, guides]
...
- [ ] **Framework Evaluation**: Run [Framework Evaluation](../../tasks/support/framework-evaluation.md) (PF-TSK-079) targeting the new task to validate completeness, consistency, and integration quality
  - **Status**: [NOT_STARTED/IN_PROGRESS/COMPLETED]
...
- [ ] Framework Evaluation (PF-TSK-079) completed for the new task
