...
function Update-FeatureQualityRankings {
    ...
    # Verify deterministic post-condition: rankings table header preserved (PF-PRO-028 v2.0)
    Assert-LineInFile -Path $TrackingFilePath -Pattern "Feature Quality Rankings" -Context "rankings header in $TrackingFilePath after update"
...
"@
    ...
    if ($PSCmdlet.ShouldProcess($OutputPath, "Write validation summary")) {
        ...
        # Verify deterministic post-condition: generation footer present (PF-PRO-028 v2.0)
        Assert-LineInFile -Path $OutputPath -Pattern "automatically generated by Generate-ValidationSummary" -Context "generation footer in $OutputPath"
    }
