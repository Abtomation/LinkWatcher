...
$templateBase = "process-framework/templates/03-testing"
...
if ($langConfig.testing.testSetup -and $langConfig.testing.testSetup.configFiles) {
    foreach ($configFile in $langConfig.testing.testSetup.configFiles) {
        ...
        $templateInLangDir = Join-Path $projectRoot "process-framework/languages-config/$($Language.ToLower())/$fileName.template"

        if (Test-Path $templateInLangDir) {
            New-FileFromTemplate `
                -TargetPath $configFile `
                -TemplatePath "process-framework/languages-config/$($Language.ToLower())/$fileName.template" `
