    ...
    switch ($ModuleName) {
        "IdRegistry" {
            if (-not $script:IdRegistryPath) {
                $script:IdRegistryPath = Join-Path -Path $projectRoot -ChildPath "process-framework/scripts/IdRegistry.psm1"
            }
...
function Get-DomainConfig {
    ...
    try {
        $projectRoot = Get-ProjectRoot
        $configPath = Join-Path -Path $projectRoot -ChildPath "process-framework/domain-config.json"
