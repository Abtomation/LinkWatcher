...
function Get-RelevantTrackingFiles {
    ...
    switch ($DocumentType) {
        "TestSpecification" {
            $trackingFiles += @(
                @{
                    Path = Join-Path $projectRoot "process-framework/state-tracking/permanent/test-implementation-tracking.md"
        ...
        "ValidationReport" {
            $trackingFiles += @(
                @{
                    Path = Join-Path $projectRoot "process-framework/state-tracking/temporary/foundational-validation-tracking-round2.md"
        ...
        "FeatureImplementation" {
            $trackingFiles += @(
                @{
                    Path = Join-Path $projectRoot "process-framework/state-tracking/permanent/feature-tracking.md"
...
function Update-FeatureTrackingStatus {
    ...
    try {
        $projectRoot = Get-ProjectRoot
        $featureTrackingPath = Join-Path $projectRoot "process-framework/state-tracking/permanent/feature-tracking.md"
            ...
            "TestSpecification" {
                ...
                try {
                    if ($DryRun) {
                        ...
                        $featureTrackingDir = Split-Path (Join-Path $projectRoot "process-framework/state-tracking/permanent/feature-tracking.md") -Parent
            ...
            "ADR" {
                ...
                try {
                    if ($DryRun) {
                        ...
                        $archTrackingPath = Join-Path $projectRoot "process-framework/state-tracking/permanent/architecture-tracking.md"
              ...
              if ($relatedFeatureId -and $relatedFeatureId -ne "" -and $relatedFeatureId -ne "TBD") {
                ...
                try {
                    ...
                    $docMapPath = Join-Path $projectRoot "process-framework/documentation-map.md"
...
function Add-TestTrackingEntry {
    ...
    try {
        $projectRoot = Get-ProjectRoot
        $timestamp = Get-ProjectTimestamp -Format "Date"
        $testTrackingPath = Join-Path $projectRoot "process-framework/state-tracking/permanent/test-implementation-tracking.md"
