...
### Transitioning FROM Feature Discovery (PF-TSK-013)
...
### Transitioning FROM System Architecture Review (PF-TSK-019)
...
### Transitioning FROM Technical Exploration (PF-TSK-093)
...
### Transitioning FROM API Design Task (PF-TSK-020)
...
### Transitioning FROM Database Schema Design Task (PF-TSK-021)
...
### Transitioning FROM FDD Creation (PF-TSK-027)
...
### Transitioning FROM Instruction Design (PF-TSK-094)
...
### Transitioning FROM Technical Design Document (TDD) Creation (PF-TSK-015)
...
### Transitioning FROM Test Specification Creation (PF-TSK-012)
...
### Transitioning FROM Core Logic Implementation (PF-TSK-078)
...
### Transitioning FROM Data Layer Implementation (PF-TSK-051)
...
### Transitioning FROM Foundation Feature Implementation Task (PF-TSK-024)
...
### Transitioning FROM Integration and Testing (PF-TSK-053)
...
### Transitioning FROM State Management Implementation (PF-TSK-056)
...
### Transitioning FROM UI Implementation (PF-TSK-052)
...
### Transitioning FROM Dimension Validation (PF-TSK-092)
...
### Transitioning FROM Bug Fixing (PF-TSK-007)
...
### Transitioning FROM Bug Triage (PF-TSK-041)
...
### Transitioning FROM Code Refactoring Task (PF-TSK-022)
...
### Transitioning FROM Code Review (PF-TSK-005)
...
### Transitioning FROM Documentation Tier Adjustment Task (PF-TSK-011)
...
### Transitioning FROM Technical Debt Assessment Task (PF-TSK-023)
...
### Transitioning FROM Framework Evaluation (PF-TSK-079)
...
### Transitioning FROM Framework Extension Task (PF-TSK-026)
...
### Transitioning FROM New Task Creation Process (PF-TSK-001)

**Lightweight Mode — Next Tasks:**

- **[Framework Evaluation](../tasks/support/framework-evaluation.md)** (PF-TSK-079) — Mandatory quality gate in a separate session before the new task can be used in production workflows
...
- [**Process Improvement**](../tasks/support/process-improvement-task.md) - If task creation reveals process gaps or improvements needed
...
### Transitioning FROM Process Improvement (PF-TSK-009)
...
### Transitioning FROM Structure Change Task (PF-TSK-014)
...
### Transitioning FROM Tools Review Task (PF-TSK-010)
...
2. **Gather Feature Context**: Ensure FDD and the tier assessment are complete
...
<!-- END HAND-WRITTEN: non-task-entries -->

<!-- BEGIN HAND-WRITTEN: tail -->
