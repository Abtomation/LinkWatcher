...
**New tool: `test_query.py`** (in `process-framework/scripts/test`):

```bash
# What tests cover feature 2.1.1?
python process-framework/scripts/test/test_query.py --feature 2.1.1

# All unit tests
python process-framework/scripts/test/test_query.py --type unit

# Test count per feature
python process-framework/scripts/test/test_query.py --summary
...
| File | Change |
|------|--------|
| `process-framework/scripts/Common-ScriptHelpers/TestTracking.psm1` | Replace `Add-TestRegistryEntry` with marker-writing function; update `Update-TestImplementationStatusEnhanced` to use file path instead of TE-TST-XXX |
| `process-framework/scripts/file-creation/03-testing/New-TestFile.ps1` | Remove registry entry creation; ensure template includes `specification` marker placeholder |
| `process-framework/scripts/validation/Validate-TestTracking.ps1` | Rewrite to validate markers (via `test_query.py --dump`) against test-tracking.md |
| `process-framework/scripts/validation/Validate-StateTracking.ps1` | Update TestTracking surface to call new validation logic |
| `process-framework/scripts/file-creation/03-testing/New-TestAuditReport.ps1` | Update TestFileId parameter to accept file path |
| `process-framework/scripts/update/Update-TestFileAuditState.ps1` | Update to use file path as identifier |
| `process-framework/scripts/update/Update-TestAuditState.ps1` | Update to use file path as identifier |

### Scripts (new)

| File | Purpose |
|------|---------|
| `process-framework/scripts/test/test_query.py` | AST-based marker reader + test counter + structured output (replaces registry as query interface) |
...
| File | Change |
|------|--------|
| `process-framework/guides/03-testing/test-infrastructure-guide.md` | Rewrite registry section → marker-based approach |
| `process-framework/guides/03-testing/integration-and-testing-usage-guide.md` | Update test-registry.yaml references |
| `process-framework/guides/03-testing/test-file-creation-guide.md` | Update new-test workflow (markers, no registry) |
| `process-framework/guides/03-testing/test-audit-usage-guide.md` | Update audit report ↔ test file linking |
| `process-framework/infrastructure/process-framework-task-registry.md` | Update file operations for affected tasks |
| `process-framework/PF-documentation-map.md` | Remove test-registry.yaml reference; add test_query.py |
| `process-framework/scripts/AUTOMATION-USAGE-GUIDE.md` | Update test tooling section |
