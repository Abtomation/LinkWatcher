...
- **[Retrospective Master State File](../../../../doc/state-tracking/temporary/old/retrospective-master-state.md)**: Tracks overall progress; read to verify Phase 2 complete, update after each session with assessment and documentation progress
...
- **[Architecture Decision Creation Guide](../../../guides/02-design/architecture-decision-creation-guide.md)**: Guide for creating Architecture Decision Records — applies to any feature with discrete architectural decisions worth recording (foundation and non-foundation alike)
...
7. **Documentation Tiers → All Documentation Tasks**: Tier determines which documents are required (Tier 2+: FDD+TDD+TestSpec+Test Migration). ADRs are decision-driven, not tier-driven — create for any feature with discrete architectural decisions worth recording.
8. **Documentation Tasks → Feature State File §4 Documentation Inventory**: All created document links (FDD / TDD / Test Spec / ADR / Schema / API / UI Design) are added as rows to each per-feature state file's `### Design Documentation` table. Master feature-tracking.md only receives Status / Test Status updates (PF-PRO-002 / PF-IMP-760)

## Implementation in AI Sessions

1. **Every Session Start**: Read [Retrospective Master State](../../../../doc/state-tracking/temporary/old/retrospective-master-state.md) to verify Phase 2 complete and identify features needing assessment/documentation
2. **Per-Feature Documentation Loop** (priority: Foundation → Tier 3 → Tier 2):
   ...
   - **Document Tier 2+ Features (continued)**:
     ...
     - Add document links to Feature Tracking
   - **Document Architectural Decisions** (any feature, not gated on tier):
     - Create ADRs using [New-ArchitectureDecision.ps1](../../../scripts/file-creation/02-design/New-ArchitectureDecision.ps1) and the [Architecture Decision Creation Guide](../../../guides/02-design/architecture-decision-creation-guide.md) for any feature where the implementation embodies a discrete architectural decision (foundation features and non-foundation features alike — mark unknowns clearly, mark "Retrospective")
     - Add document links to Feature Tracking
