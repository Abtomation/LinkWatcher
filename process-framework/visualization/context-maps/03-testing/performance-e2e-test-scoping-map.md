...
- **Performance Tests Needed?**: Decision point — results in new `⬜ Specified` entries in performance-test-tracking.md or documented "not needed" rationale
...
- **Performance Test Creation (PF-TSK-084)**: Downstream task that implements performance tests from `⬜ Specified` entries
...
3. **This Task → Performance Test Tracking**: Adds `⬜ Specified` entries when performance tests are needed
4. **This Task → E2E Test Tracking**: Adds entries when a workflow becomes E2E-ready
5. **Performance Test Tracking → PF-TSK-084**: New `⬜ Specified` entries trigger Performance Test Creation
