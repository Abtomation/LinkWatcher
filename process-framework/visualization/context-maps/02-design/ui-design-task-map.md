...
description: "Components for UI/UX design (wireframes, visual specs, accessibility, platform adaptations)"
---

# UI Design Task Context Map

This context map provides a visual guide to the components and relationships relevant to the UI Design Task ([PF-TSK-090](../../../tasks/02-design/ui-design-task.md)). Use this map to identify which components require attention and how they interact.
...
graph TD
    ...
    FTA([Feature Tier Assessment]) --> UIDesign([UI Design Task])
...
- **UI Design Task**: Core task definition that orchestrates the visual and interaction design process for features
...
- **Feature State File §4 Documentation Inventory**: Per-feature state file (`doc/state-tracking/features/<id>-implementation-state.md`) where UI Design rows are inserted by `New-UIDesign.ps1` (PF-PRO-002 / PF-IMP-760)
...
1. **Feature Tier Assessment → UI Design Task**: UI Design is typically created for Tier 2+ features or when visual complexity requires upfront design
2. **UI Design Task → Design Guidelines**: Task MUST consult Design Guidelines (PD-UIX-001) to ensure consistency with established patterns, colors, typography, spacing, and accessibility standards
3. **UI Design Task → New-UIDesign.ps1 Script**: Task execution uses the script to create structured UI Design documents with proper metadata and Design Guidelines reference
...
3. **Execute UI Design Task**: Use the task definition to understand AI agent role (Designer/Visual Design Specialist) and responsibilities
...
7. **State File Documentation Inventory Updated**: `New-UIDesign.ps1` automatically inserts a row into the per-feature state file's §4 Documentation Inventory (PF-PRO-002 / PF-IMP-760). The master feature-tracking.md row only receives a Status update.
...
- [Feature Tracking](../../../../doc/state-tracking/permanent/feature-tracking.md) - Lightweight cross-feature index (Status / Priority / Doc Tier / Test Status / Dependencies / Notes). Per-feature design artifact tracking lives in each feature's `doc/state-tracking/features/<id>-implementation-state.md` §4 Documentation Inventory (PF-PRO-002).
- [New-UIDesign.ps1 Script](../../../scripts/file-creation/02-design/New-UIDesign.ps1) - Script for creating UI Design documents

---

_This context map provides a complete view of the UI Design Task ecosystem. Use it to understand component relationships and ensure proper task execution. The visual diagram emphasizes the critical role of Design Guidelines (PD-UIX-001) consultation in maintaining design consistency._
