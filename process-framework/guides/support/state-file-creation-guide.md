...
description: "Guide for customizing state tracking file templates"
---
...
- Access to PowerShell and the New-PermanentState.ps1 script in `process-framework/scripts/file-creation/support`
- Understanding of what project component or process needs state tracking
- Familiarity with existing permanent state files in `doc/state-tracking/permanent`
...
- **Test Tracking**: Monitor test case implementation, coverage, and manual test execution
...
3. **Verify state file creation**:
   ...
   - Note the file path in `doc/state-tracking/permanent`
...
1. Ensure you're running from `doc/state-tracking` directory
...
- [Process Improvement Tracking](../../../process-framework-central/state-tracking/permanent/process-improvement-tracking.md) - Example cross-project tracking (central)
