...
**Relationship to validation**: The [Feature Validation Guide](../05-validation/feature-validation-guide.md) references this guide for dimension definitions and applicability criteria but retains its own scoring methodology, thresholds, and reporting templates. AI Agent Continuity (run via [Dimension Validation](../../tasks/05-validation/dimension-validation-task.md), PF-TSK-092, the [ai-agent-continuity path](../../tasks/05-validation/ai-agent-continuity-validation-path.md)) is a standalone validation task not backed by a dimension.
