...
def read_runtime_dependencies(project_root):
    """Return the [project] dependencies declared in pyproject.toml.

    The install tree is *flat* (`<install>/linkwatcher/`) while pyproject
    declares a src layout (`where = ["src"]`), so `pip install <install_dir>`
    cannot build it — it fails with "error in 'egg_base' option: 'src' does not
    exist". Emitting a requirements.txt from these values lets
    create_linkwatcher_venv take its requirements branch and install the
    declared dependencies directly, with no build step at all.

    Uses tomllib where available (3.11+) and falls back to a line scan of the
    dependencies array, so the installer keeps working on the 3.8+ it claims.
    Returns [] if the list cannot be read; the caller then leaves the existing
    behavior untouched rather than writing an empty requirements file.
    """
    ...
    try:
        ...
        with pyproject.open("rb") as handle:
            return list(tomllib.load(handle).get("project", {}).get("dependencies", []))
    except ImportError:
        pass
    ...
    # Fallback: scan the top-level `dependencies = [ ... ]` array. The
    # optional-dependencies tables are section headers, never assignments, so
    # a line-anchored match cannot pick them up.
    try:
...
def write_requirements_file(project_root, install_dir):
    """Write the install dir's requirements.txt from the declared runtime deps.

    Consumed by create_linkwatcher_venv, which prefers requirements.txt over
    building the (unbuildable) flat install tree.
    """
    dependencies = read_runtime_dependencies(project_root)
    if not dependencies:
        print("   WARNING: No runtime dependencies found in pyproject.toml")
        return False

    body = "\n".join(dependencies)
    (install_dir / "requirements.txt").write_text(
        "# Generated by deployment/install_global.py from pyproject.toml\n"
        "# [project] dependencies. Do not edit here — edit pyproject.toml.\n"
        f"{body}\n",
        encoding="utf-8",
    )
    print(f"   Generated: requirements.txt ({len(dependencies)} runtime dependencies)")
    return True
...
def install_linkwatcher(project_root, install_dir):
    ...
    # control_panel.py is the Control Panel entry script (feature 7.1.1); it
    # sits beside main.py, and the linkwatcher_control_panel subpackage it
    # imports already ships inside the src/linkwatcher tree copied below.
    core_files = [
        "main.py",
        "control_panel.py",
        "pyproject.toml",
    ...
    # Must run after the copies: create_linkwatcher_venv reads this file from
    # the install dir to install dependencies without building the flat tree.
    write_requirements_file(project_root, install_dir)
...
def create_linkwatcher_venv(install_dir, max_attempts=2):
    ...
    try:
        result = subprocess.run(
            # Assert every declared runtime dependency, so a venv that cannot
            # actually run the product fails here rather than at first use.
            # structlog/psutil were missing from this list while being real
            # imports (main.py, the Control Panel's discovery).
            [
                str(venv_python),
                "-c",
                "import watchdog, yaml, git, colorama, structlog, psutil; print('OK')",
            ],
            capture_output=True,
            text=True,
        )
...
def create_wrapper_scripts(install_dir):
    ...
    # Control Panel wrappers (feature 7.1.1). These use the console python.exe,
    # not pythonw.exe: a terminal launch must be able to show the panel's
    # "already running — surfaced that window" message. The SessionStart hook
    # uses pythonw.exe instead, where no console is wanted.
    wrappers["linkwatcher-panel.bat"] = (
        f"@echo off\n"
        f"REM LinkWatcher Control Panel (feature 7.1.1)\n"
        f'set "LWPYTHON=%~dp0{venv_python_rel}"\n'
        f'if not exist "%LWPYTHON%" (\n'
        f"    echo Error: LinkWatcher venv not found."
        f" Run: python deployment/install_global.py\n"
        f"    exit /b 1\n"
        f")\n"
        f'"%LWPYTHON%" "%~dp0control_panel.py" %*\n'
    )
    wrappers["linkwatcher-panel.ps1"] = (
        f"# LinkWatcher Control Panel (feature 7.1.1)\n"
        f'$lwVenvPython = Join-Path $PSScriptRoot "{venv_python_rel}"\n'
        f"if (-not (Test-Path $lwVenvPython)) {{\n"
        f'    Write-Host "Error: LinkWatcher venv not found. Run: python deployment/install_global.py" -ForegroundColor Red\n'  # noqa: E501
        f"    exit 1\n"
        f"}}\n"
        f'& $lwVenvPython "$PSScriptRoot\\control_panel.py" @args\n'
    )
...
def test_installation(install_dir):
    ...
    try:
        ...
        # Control Panel smoke test (feature 7.1.1). --help is GUI-safe: argparse
        # prints and exits before any Tk window or single-instance socket exists.
        panel_script = install_dir / "control_panel.py"
        if panel_script.exists():
            panel_result = subprocess.run(
                [python_exe, str(panel_script), "--help"],
                capture_output=True,
                text=True,
            )
