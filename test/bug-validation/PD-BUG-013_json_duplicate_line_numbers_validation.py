  ...
  "backup": {
    "data_file": "data.csv"
        ...
        if unique_lines == len(data_refs) and len(data_refs) == 4:
            print(
                f"\n[PASS] Check 1: Parser found {len(data_refs)} refs with {unique_lines} unique line numbers: {sorted(line_numbers)}"  # noqa: E501
            )
            checks_passed += 1
        else:
            print(
                f"\n[FAIL] Check 1: Expected 4 refs with 4 unique lines, got {len(data_refs)} refs with lines {line_numbers}"  # noqa: E501
            )
        ...
        if "datasets/main.csv" in updated_content:
            print("[PASS] Check 3: New path 'datasets/main.csv' found in updated content")
            checks_passed += 1
        else:
            print("[FAIL] Check 3: New path 'datasets/main.csv' NOT found in updated content")

        # --- Check 4: Valid JSON structure maintained ---
        try:
            updated_data = json.loads(updated_content)
            print("[PASS] Check 4: Updated content is valid JSON")
            checks_passed += 1
        ...
        if updated_data:
            ...
            if all_updated:
                print("[PASS] Check 5: All 4 JSON values updated to new path")
                checks_passed += 1
            else:
                print("[FAIL] Check 5: Not all values updated:")
