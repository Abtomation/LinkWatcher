"""
Scale Tests for Large Projects (PH Test Cases, Level 3)

This module implements the scale-level PH test cases, focusing on performance with
large projects and many files.
...
Split from the original test_large_projects.py (TD254): resource-bound tests
(PH-007 memory, PH-008 CPU) live in level4-resource/test_resource_bounds.py. The
shared warmup helper is the `warmup_service` factory fixture in performance/conftest.py.
"""
...
class TestLargeProjectHandling:
    ...
    @pytest.mark.slow
    def test_ph_001_thousand_plus_files(self, temp_project_dir, warmup_service):
        """
        PH-001: 1000+ files with links
    ...
    def test_ph_002_deep_directory_structures(self, temp_project_dir, warmup_service):
        """
        PH-002: Deep directory structures
    ...
    @pytest.mark.slow
    def test_ph_003_large_files(self, temp_project_dir, warmup_service):
        """
        PH-003: Large files
    ...
    def test_ph_004_many_references_to_single_file(self, temp_project_dir, warmup_service):
        """
        PH-004: Many references to single file
    ...
    @pytest.mark.slow
    def test_ph_005_rapid_file_operations(self, temp_project_dir, warmup_service):
        """
        PH-005: Rapid file operations
        ...
        Priority: High
        """
        warmup_service(num_files=5, num_moves=3)
...
class TestDirectoryBatchDetection:
    ...
    @pytest.mark.slow
    def test_ph_006_directory_batch_detection(self, temp_project_dir, warmup_service):
        """
        PH-006: Directory batch detection at scale
        ...
        """
        warmup_service(num_files=5, dir_move=True)
