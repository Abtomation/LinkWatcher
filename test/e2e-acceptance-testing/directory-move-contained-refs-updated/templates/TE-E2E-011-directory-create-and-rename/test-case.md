---
id: TE-E2E-011
description: "E2E acceptance test (WF-002): TE-E2E-011 Directory Create and Rename."
type: Testing
...
# Test Case: TE-E2E-011 Directory Create and Rename

## Preconditions

- [ ] LinkWatcher is **stopped** before workspace setup, then **started** after setup (fresh scan)
- [ ] Test environment set up via `Setup-TestEnvironment.ps1 -Workflow directory-move-contained-refs-updated`
...
3. **Rename the directory**: Rename `project/utils/` to `project/tools/` (same parent, different name)
   - **Tool**: File Explorer (right-click → Rename) or Command Line
   - **Target**: `project/utils/` → `project/tools/`

4. **Wait for LinkWatcher to process**: Allow time for directory rename detection and reference updates
   - **Duration**: Wait 5–10 seconds
   - **Observe**: LinkWatcher log should show directory rename detection and reference updates for all contained files

5. **Verify references updated**: Check that README.md now points to files in `tools/`
   - **Tool**: Text editor
...
**Script**: `run.ps1`
**Action**: Creates `project/utils/` with `helper.py` and `config.yaml`, waits 5 seconds, then renames the directory to `project/tools/`
...
|------|-------------|--------|-------|
| `README.md` | Line 3 | `[Helper](utils/helper.py)` | `[Helper](tools/helper.py)` |
| `README.md` | Line 5 | `[Config](utils/config.yaml)` | `[Config](tools/config.yaml)` |
...
- LinkWatcher log shows directory rename detection from `utils/` to `tools/`
- LinkWatcher log shows updates to `README.md` for both contained files
- No errors or warnings in LinkWatcher log

## Verification Method

- [ ] **Automated comparison**: Run `Verify-TestResult.ps1 -TestCase TE-E2E-011 -Workflow directory-move-contained-refs-updated` — compares workspace against `expected/`
- [ ] **Log check**: Check LinkWatcher log for directory rename detection and update messages without errors

## Pass Criteria

- [ ] Both references in README.md updated from `utils/` paths to `tools/` paths
- [ ] `helper.py` and `config.yaml` exist under `tools/` with original content
...
- This tests in-place directory rename (same parent, different name) as opposed to a cross-directory move
- Directory renames generate multiple delete+create events for all contained files; LinkWatcher's dir_move_detector must group them correctly
- Complements TE-E2E-009 which tests cross-directory move of a directory
