...
#
# Usage (orchestrated):
#   Run-E2EAcceptanceTest.ps1 -TestCase "TE-E2E-012" -Workflow "startup-initial-project-scan"
...
# Step 7: Allow LinkWatcher to process the startup-race move, then stop the instance this
# script started. Run-E2EAcceptanceTest.ps1 v2.0 no longer stops LinkWatcher for us — leaving
# this process running blocks the next test case's -Clean setup (it holds the workspace open).
Start-Sleep -Seconds 12
if ($lwProcess) { Stop-Process -Id $lwProcess.Id -Force -ErrorAction SilentlyContinue }
