...
test_file_path: test/automated/performance/level3-scale/test_large_projects.py
feature_id: 0.1.1
auditor: AI Agent
---
...
> **⚠️ Partially superseded by the TD254 split (2026-06-04).** PH-007 (memory) and PH-008 (CPU) have moved from this file to [test_resource_bounds.py](/test/automated/performance/level4-resource/test_resource_bounds.py); PH-001..006 remain in [test_large_projects.py](/test/automated/performance/level3-scale/test_large_projects.py). Retained as the historical record of the 2026-04-29 audit; a fresh per-level re-audit is required (flagged 🔄 in [performance-test-tracking.md](/test/state-tracking/permanent/performance-test-tracking.md)).
...
- **Timing precision**: All 8 PH tests now use `time.perf_counter()` (24 call sites verified at [test_large_projects.py:99, 124, 130, 132, 152, 156, 204, 206, 219, 223, 271, 273, 284, 286, 325, 344, 350, 352, 366, 370, 424, 436, 518, 527](/test/automated/performance/level3-scale/test_large_projects.py)). `time.time()` is no longer used anywhere in the file. Resolution is sub-µs and monotonic — meaningful for the smallest measurements (PH-002 move at 50 ms). Resolves TE-TAR-070 Critical Issue 2 / TD244 (PD-REF-216 mirror work) — verified.
- **Warmup cycles**: All 8 PH tests now invoke a shared `_warmup_service()` helper at [test_large_projects.py:29-61](/test/automated/performance/level3-scale/test_large_projects.py) before any timed window:
...
- **PH-005 sleep contamination removed**: `time.sleep(0.01)` no longer appears in PH-005's loop ([test_large_projects.py:424-436](/test/automated/performance/level3-scale/test_large_projects.py)). PH-005 totals are now 3.76–3.89 s (vs 5.32 s pre-rework Run 1 of TE-TAR-070); the artificial 0.5 s noise floor is gone. Resolves TE-TAR-070 Improvement Opportunity / TD248 (PD-REF-218) — verified.
- **PH-008 measures process CPU, not system-wide**: [test_large_projects.py:625, 629](/test/automated/performance/level3-scale/test_large_projects.py) — `process = psutil.Process(os.getpid())` then `process.cpu_percent(interval=0.1)` inside the monitor thread. Mirrors PH-007's pattern. The peak assertion is removed (diagnostic only), and the avg assertion is rewritten as `(avg_cpu / cpu_count) < 80` at [test_large_projects.py:680-681](/test/automated/performance/level3-scale/test_large_projects.py) to keep the [0,100] semantics on multi-core hosts. With 16 logical cores, this audit's three PH-008 raw avgs (55.6 / 56.4 / 71.5%) normalize to 3.5–4.5% — well below 80%. The peak readings (133 / 118 / 118%) are above 100% (cores × 100% scale) but are now diagnostic-only and do not gate the assertion. Resolves TE-TAR-070 Critical Issue 1 / TD247 (PD-REF-217) — verified.
...
- **Test class docstring** at [test_large_projects.py:7-15](/test/automated/performance/level3-scale/test_large_projects.py) now lists all 8 tests including PH-006 (a doc-drift fix bundled with the rework). Resolves TE-TAR-070 Improvement Opportunity (docstring drift) — verified.
...
- **Module-level pytest markers**: `feature("4.1.1")`, `priority("Extended")`, `cross_cutting(["0.1.1", "1.1.1", "2.2.1"])`, `test_type("performance")` at [test_large_projects.py:64-69](/test/automated/performance/level3-scale/test_large_projects.py). Per-test markers `@pytest.mark.slow` on PH-001/PH-003/PH-005/PH-006. ✅
...
This is a re-audit of [TE-TAR-070](audit-report-0-1-1-test-large-projects-2026-04-29-TE-TAR-070.md) after the methodology rework that mirror the BM-tests' PD-REF-196 work plus 3 PH-specific fixes. All 6 tech debt items registered by TE-TAR-070 are resolved and verified by the current test file:
