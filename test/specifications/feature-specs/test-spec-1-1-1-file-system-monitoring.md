...
| Own-output exclusion (PD-BUG-107) | Daemon never indexes or reacts to its own log outputs | `TestOwnOutputExclusion` — modify on own log / colocated output not indexed (breaks the rescan loop); rotation rename does not rewrite docs referencing the stable log path; creates/deletes in the output zone not indexed/buffered. `TestOwnOutputPredicate` — exclusion registry unit tests (log-dir zone, project-root fallback to rotation family, no-file-logging inert; PD-BUG-109 — a log dir outside the project root (ancestor, sibling, or root-prefix lookalike) yields an empty registry, never an exclusion that swallows the watched tree; a log dir strictly inside a drive-root project keeps its exclusion despite abspath's trailing separator on drive roots). `TestOwnOutputScanExclusion` (in test_service.py, feature 0.1.1) — initial scan skips the own-output zone | Manual setup |
...
**Test File**: [`test/automated/integration/test_file_movement.py`](../../automated/unit/1-file-watching-detection/1-0-file-watching-detection/test_file_movement.py) (7 methods)
...
**Test File**: [`test/automated/integration/test_sequential_moves.py`](../../automated/unit/1-file-watching-detection/1-0-file-watching-detection/test_sequential_moves.py) (4 methods)
...
**Test File**: [`test/automated/unit/test_comprehensive_file_monitoring.py`](../../automated/unit/1-file-watching-detection/1-0-file-watching-detection/test_comprehensive_file_monitoring.py) (7 methods)
...
**Test File**: [`test/automated/integration/test_image_file_monitoring.py`](../../automated/unit/1-file-watching-detection/1-0-file-watching-detection/test_image_file_monitoring.py) (6 methods)
...
**Test File**: [`test/automated/integration/test_powershell_script_monitoring.py`](../../automated/unit/1-file-watching-detection/1-0-file-watching-detection/test_powershell_script_monitoring.py) (5 methods)
...
**Test File**: [`test/automated/test_directory_move_detection.py`](../../automated/unit/1-file-watching-detection/1-0-file-watching-detection/test_directory_move_detection.py) (18 methods)
