...
tdd_path: doc/technical/tdd/tdd-5-1-1-cicd-development-tooling-t2.md
test_tier: 2
retrospective: true
---
...
This document provides comprehensive test specifications for the **CI/CD & Development Tooling** feature (ID: 5.1.1), derived from the Technical Design Document [PD-TDD-031](/doc/technical/tdd/tdd-5-1-1-cicd-development-tooling-t2.md).

**Test Tier**: 2 (Pipeline-level validation)
**TDD Reference**: [TDD PD-TDD-031](/doc/technical/tdd/tdd-5-1-1-cicd-development-tooling-t2.md)
...
CI/CD & Development Tooling encompasses the GitHub Actions pipeline (`.github/workflows/ci.yml`), test automation (`Run-Tests.ps1`), code quality tools (flake8, black, isort, mypy), coverage reporting (pytest-cov + Codecov), pre-commit hooks, package building (`pyproject.toml` + `setup.py`), and Windows dev scripts (`dev.bat` + `Makefile`).
...
|---------------|-----------------|------------|
| `Run-Tests.ps1 -Discover` | Prints test count without executing | Low |
| `Run-Tests.ps1 -Category unit` | Only executes `test/automated/unit/` directory | Low |
| `Run-Tests.ps1 -Category parsers` | Only executes `test/automated/parsers/` directory | Low |
| `Run-Tests.ps1 -Category integration` | Only executes `test/automated/integration/` directory | Low |
| `Run-Tests.ps1 -Performance` | Only executes `test/automated/performance/` directory | Low |
| `Run-Tests.ps1 -Coverage` | Generates coverage report | Medium |
...
2. **Run-Tests.ps1 smoke test**: A test that invokes `Run-Tests.ps1 -Discover` and verifies output format
...
No mocks needed for pipeline-level validation. If `Run-Tests.ps1` tests are created, they would need:
- Subprocess execution to invoke `Run-Tests.ps1` with various flags
...
- **TDD**: [`doc/technical/tdd/tdd-5-1-1-cicd-development-tooling-t2.md`](/doc/technical/tdd/tdd-5-1-1-cicd-development-tooling-t2.md)
