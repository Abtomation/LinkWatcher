...
tdd_path: doc/technical/tdd/tdd-4-1-1-test-suite-t2.md
test_tier: 2
retrospective: true
---
...
This document provides comprehensive test specifications for the **Test Suite** feature (ID: 4.1.1), derived from the Technical Design Document [PD-TDD-027](/doc/technical/tdd/tdd-4-1-1-test-suite-t2.md).

**Test Tier**: 2 (Infrastructure validation + performance)
**TDD Reference**: [TDD PD-TDD-027](/doc/technical/tdd/tdd-4-1-1-test-suite-t2.md)
...
The Test Suite infrastructure consists of pytest configuration (`pytest.ini`), shared fixtures (`conftest.py`), test utilities (`utils.py`), the process framework test runner (`Run-Tests.ps1`), test configuration presets (`test/automated/test_config.py`), and performance benchmarks. It provides the foundation for all 328 test methods across 4 categories (unit, integration, parser, performance).
...
- `Run-Tests.ps1 -Category unit` executes only `test/automated/unit/`
...
3. **Low Priority** (Gaps identified)
   - [ ] `Run-Tests.ps1` execution modes (9 modes including multi-category — not tested via test code)
   - [ ] Marker-based filtering (`@pytest.mark.slow` registered but not in pytest config)
   - [ ] Category execution isolation (`-Category unit` only runs `test/automated/unit/`)
...
- **CLI test runner**: `Run-Tests.ps1` has 9 execution modes (including multi-category) but none are tested programmatically
...
- **TDD**: [`doc/technical/tdd/tdd-4-1-1-test-suite-t2.md`](/doc/technical/tdd/tdd-4-1-1-test-suite-t2.md)
