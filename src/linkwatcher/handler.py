...
class LinkMaintenanceHandler(FileSystemEventHandler):
    ...
    def _handle_directory_moved(self, event: FileMovedEvent):
        ...
        try:
            ...
            # PD-BUG-091: propagate the per-file update count into the total
            # so directory_move_completed accurately reports all work done.
            phase_1_5_refs_updated = 0
            for old_file_path, new_file_path in moved_files:
                abs_new_path = os.path.join(str(self.project_root), new_file_path)
                phase_1_5_refs_updated += self._update_links_within_moved_file(
            ...
            self.logger.info(
                ...
                within_moved_files_refs_updated=phase_1_5_refs_updated,
                dir_path_refs_updated=dir_refs_updated,
            )
            # PD-BUG-091: Phase 1.5 counts were already added to links_updated
            # inside _update_links_within_moved_file, so only add Phase 1b + 2
            # here to avoid double-counting.
            self._update_stat("links_updated", total_references_updated - phase_1_5_refs_updated)
    ...
    def _update_links_within_moved_file(
        ...
        Returns the number of links updated (0 if none or on error).
        """
