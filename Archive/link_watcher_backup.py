...
try:
    ...
    from watchdog.events import FileSystemEventHandler, FileMovedEvent, FileDeletedEvent, FileCreatedEvent
    import yaml
    import markdown
...
class LinkParser:
    ...
    def _extract_yaml_file_refs(self, data, file_path: str, lines: List[str], references: List[LinkReference], path=""):
    ...
    def _extract_json_file_refs(self, data, file_path: str, lines: List[str], references: List[LinkReference], path="", processed_values=None):
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                file_related_keys = {
                    'templatepath', 'template', 'script', 'file', 'path', 'filename', 
                    'src', 'source', 'target', 'destination', 'include', 'import',
                    'defaulttemplate', 'configfile', 'datafile', 'outputfile'
                }
                ...
                if (key.lower() in file_related_keys or key_suggests_file) and isinstance(value, str):
                    ...
                    self._extract_json_file_refs(value, file_path, lines, references, current_path, processed_values)
                
        elif isinstance(data, list):
            for i, item in enumerate(data):
                current_path = f"{path}[{i}]" if path else f"[{i}]"
                self._extract_json_file_refs(item, file_path, lines, references, current_path, processed_values)
    ...
    def _parse_generic(self, file_path: str, content: str) -> List[LinkReference]:
        ...
        import_pattern = re.compile(r'(?:import|require|include|from)\s+[\'"]([a-zA-Z0-9_\-./\\]+\.[a-zA-Z0-9]+)[\'"]')
        ...
        unquoted_pattern = re.compile(r'(?:^|\s)([a-zA-Z0-9_\-]+/[a-zA-Z0-9_\-./\\]*\.[a-zA-Z0-9]+)(?:\s|$)')
...
class LinkUpdater:
    ...
    def update_references(self, references: List[LinkReference], old_path: str, new_path: str) -> Dict[str, int]:
    ...
    def _update_file(self, file_path: str, references: List[LinkReference], old_path: str, new_path: str) -> int:
        ...
        try:
            ...
            for ref in references:
                if ref.line_number <= len(lines):
                    ...
                    new_target = self._calculate_new_target_relative(ref.link_target, old_path, new_path, ref.file_path)
    ...
    def _calculate_new_target_relative(self, original_target: str, old_path: str, new_path: str, source_file: str) -> str:
        """Calculate the new target path, considering relative paths from the source file."""
        # Handle anchors
        if '#' in original_target:
            target_part, anchor = original_target.split('#', 1)
            new_target_part = self._calculate_relative_path(target_part, old_path, new_path, source_file)
            return f"{new_target_part}#{anchor}"
        else:
    ...
    def _target_matches_old_path_simple(self, target_norm: str, old_norm: str, source_dir: str) -> bool:
...
class LinkMaintenanceHandler(FileSystemEventHandler):
    ...
    def _get_file_hash(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            if os.path.exists(file_path) and os.path.getsize(file_path) < 1024 * 1024:  # Only hash files < 1MB
    ...
    def _detect_move_operation(self, created_path: str) -> Optional[str]:
        ...
        if filename in self.recent_deletions:
            ...
            self._debug_log(f"Found recent deletion: {old_path} (deleted {time_since_deletion:.2f}s ago)")
            ...
            if time_since_deletion < self.move_detection_window:
                ...
                del self.recent_deletions[filename]
                self._debug_log(f"DETECTED MOVE: {old_path} -> {created_path} (filename + timing match, {time_since_deletion:.2f}s)")
                return old_path
            else:
                self._debug_log(f"Time window exceeded ({time_since_deletion:.2f}s > {self.move_detection_window}s)")
        else:
    ...
    def on_moved(self, event):
        ...
        try:
            # Debug logging
            self._debug_log(f"MOVED event: {event.src_path} -> {event.dest_path} (dir: {event.is_directory})")
        ...
        try:
            ...
            if not event.is_directory and should_monitor:
                ...
                if rel_path:
                    ...
                    self._debug_log(f"Stored deletion info for move detection: {filename} -> {rel_path} (hash: {file_hash})")
            ...
            if not event.is_directory and self._should_monitor_file(event.src_path):
                ...
                if old_path:
                    ...
                    if new_rel:
                        ...
                        self._handle_file_moved(os.path.join(str(self.project_root), old_path), event.src_path)
                    return
    ...
    def _handle_directory_moved(self, old_path: str, new_path: str):
        ...
        if all_references:
            ...
            for ref in all_references:
                ...
                if old_target.startswith(old_rel + '/') or old_target.startswith('/' + old_rel + '/'):
...
class LinkWatcherService:
    ...
    def start(self, initial_scan: bool = True):
        ...
        if initial_scan:
            ...
            self._initial_scan()
            print(f"{Fore.GREEN}Initial scan complete. Found {len(self.link_db.files_with_links)} files with links.")
        ...
        try:
            ...
            if ctrl_c_count == 0 or (current_time - last_ctrl_c_time) > 2:
                ...
                try:
                    ...
                    print(f"{Fore.GREEN}Continuing to monitor... (Press Ctrl+C twice quickly to stop)")
...
def main():
    ...
    parser.add_argument("--no-initial-scan", action="store_true", 
                       help="Skip initial scan of files")
