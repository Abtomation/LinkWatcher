...
- **5.1.1-FR-5**: The system SHALL provide local environment bootstrapping via `dev.bat dev-setup` (installs dependencies and pre-commit hooks)
...
- **5.1.5-FR-3**: The system SHALL support installation via `pre-commit install` and `pre-commit install --hook-type commit-msg`, automated through `dev.bat pre-commit` target
...
- **5.1.5-AC-2**: `pre-commit install` is automated by `dev.bat dev-setup` / `dev.bat pre-commit`
