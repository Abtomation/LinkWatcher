...
- **4.1.1-FR-4**: The system SHALL provide category-based test execution via the process framework runner (`Run-Tests.ps1`) with flags: `-Category unit`, `-Category integration`, `-Category parsers`, `-Performance`, `-Critical`, `-Quick`, `-All`, `-Coverage`, `-Discover`, `-Lint` (supports multi-category: `-Category unit,integration`)
...
- **4.1.1-AC-2**: `Run-Tests.ps1 -Category unit` executes only tests in `test/automated/unit/`
...
- **4.1.3-AC-4**: Runnable via `Run-Tests.ps1 -Category integration` or `pytest test/automated/integration/`
...
- **4.1.4-AC-3**: Runnable via `Run-Tests.ps1 -Category parsers` or `pytest test/automated/parsers/`
