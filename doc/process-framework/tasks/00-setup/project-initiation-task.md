...
9. **Set Up Testing Infrastructure**: Run the bootstrapping script to scaffold the test environment:
   ...
   ```
   This creates: test directory structure, tracking files (`test-tracking.md`, `e2e-test-tracking.md`), `TE-id-registry.json`, shared fixtures, and package markers. See the [Test Infrastructure Guide](/doc/process-framework/guides/03-testing/test-infrastructure-guide.md) for details.
