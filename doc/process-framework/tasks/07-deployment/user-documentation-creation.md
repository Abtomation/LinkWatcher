...
- **Critical (Must Read):**
  ...
  - [Feature Tracking](../../../product-docs/state-tracking/permanent/feature-tracking.md) — Identify which features need user documentation
...
6. **Create or update handbook**:
   - **For new handbooks**: Use the creation script:
     ```bash
     cd /c/path/to/project/doc/process-framework/scripts/file-creation/07-deployment && pwsh.exe -ExecutionPolicy Bypass -Command '& .\New-Handbook.ps1 -HandbookName "Feature Name" -Description "What users can do with this feature" -Confirm:$false'
     ```
