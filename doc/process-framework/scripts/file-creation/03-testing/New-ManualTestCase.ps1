...
try {
    ...
    if ($NewGroup) {
        ...
        $masterTemplatePath = Join-Path $projectRoot "doc/process-framework/templates/03-testing/manual-master-test-template.md"
    ...
    $testCaseTemplatePath = Join-Path $projectRoot "doc/process-framework/templates/03-testing/manual-test-case-template.md"
    ...
    if (-not $OpenInEditor) {
        $details += @(
            ...
            "See: doc/process-framework/guides/03-testing/manual-test-case-customization-guide.md"
        )
    }
