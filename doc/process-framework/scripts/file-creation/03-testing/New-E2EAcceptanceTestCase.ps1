...
try {
    ...
    if ($NewGroup) {
        ...
        $masterTemplatePath = Join-Path $projectRoot "doc/process-framework/templates/03-testing/e2e-acceptance-master-test-template.md"
...
"@
    ...
    $testCaseTemplatePath = Join-Path $projectRoot "doc/process-framework/templates/03-testing/e2e-acceptance-test-case-template.md"
...
"@
    ...
    if (-not $OpenInEditor) {
        $details += @(
            ...
            "See: doc/process-framework/guides/03-testing/e2e-acceptance-test-case-customization-guide.md"
        )
    }
