...
if ($CrossCutting) {
    $templatePath = Join-Path $projectRoot "doc/process-framework/templates/03-testing/cross-cutting-test-specification-template.md"
    $outputDirectory = Join-Path $projectRoot "test/specifications/cross-cutting-specs"
    $documentName = "cross-cutting-spec-$FeatureName"
} else {
    $templatePath = Join-Path $projectRoot "doc/process-framework/templates/03-testing/test-specification-template.md"
...
"@
    ...
    if (-not $OpenInEditor) {
        $details += @(
            ...
            "doc/process-framework/guides/03-testing/test-specification-creation-guide.md",
