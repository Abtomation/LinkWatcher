...
try {
    $projectRoot = Get-ProjectRoot
    $templatePath = Join-Path $projectRoot "doc/process-framework/templates/02-design/architecture-impact-assessment-template.md"
    ...
    if (-not $OpenInEditor) {
        $details += @(
            ...
            "doc/process-framework/guides/02-design/architecture-assessment-creation-guide.md",
