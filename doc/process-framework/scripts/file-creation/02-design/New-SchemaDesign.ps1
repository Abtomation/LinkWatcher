...
try {
    ...
    $documentId = New-StandardProjectDocument -TemplatePath "doc/process-framework/templates/02-design/schema-design-template.md" -IdPrefix "PD-SCH" -IdDescription "Schema design for $SchemaType changes in ${FeatureName}" -DocumentName $documentName -DirectoryType "schemas" -Replacements $customReplacements -AdditionalMetadataFields $additionalMetadataFields -OpenInEditor:$OpenInEditor
    ...
    if (-not $OpenInEditor) {
        $details += @(
            ...
            "doc/process-framework/guides/02-design/schema-design-creation-guide.md",
