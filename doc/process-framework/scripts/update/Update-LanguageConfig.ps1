...
if (-not $projectRoot -or -not (Test-Path (Join-Path $projectRoot "doc/process-framework/project-config.json"))) {
    # Search upward from CWD for project-config.json
    $searchDir = (Get-Location).Path
    while ($searchDir -and $searchDir.Length -gt 3) {
        if (Test-Path (Join-Path $searchDir "doc/process-framework/project-config.json")) {
...
if (-not $projectRoot) {
    Write-Error "Could not find project root (no doc/process-framework/project-config.json found)"
    exit 1
}
$langConfigDir = Join-Path $projectRoot "doc/process-framework/languages-config"
$templatePath = Join-Path $projectRoot "doc/process-framework/templates/support/language-config-template.json"
