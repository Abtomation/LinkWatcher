...
<#
.SYNOPSIS
    Sets up the manual test execution environment by copying pristine fixtures into workspace.

.DESCRIPTION
    Copies test case fixtures from test/manual-testing/templates/ into test/manual-testing/workspace/
...
foreach ($grp in $groups) {
    ...
    # Find all test case directories (MT-NNN-*)
    $testCases = Get-ChildItem $srcGroup -Directory | Where-Object { $_.Name -match '^MT-\d+' }
