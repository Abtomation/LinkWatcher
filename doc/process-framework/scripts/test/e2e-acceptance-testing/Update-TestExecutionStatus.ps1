...
.PARAMETER FeatureId
    Optional: Mark all E2E acceptance test groups/cases for a feature.

.PARAMETER Group
    Optional: Mark a specific test group by name.

.PARAMETER TestCase
    Optional: Mark a specific test case by ID (e.g., "E2E-001").
...
.EXAMPLE
    .\Update-TestExecutionStatus.ps1 -TestCase "E2E-001" -Status "Failed" -Reason "Link not updated after rename"
...
foreach ($line in $lines) {
    ...
    if ($line -match '^\|') {
        ...
        if ($cells.Count -ge 6) {
            ...
            # Match by group (check if testId matches E2E-GRP pattern and test type is "E2E Group")
            if ($Group -and $testType -match 'E2E' -and $line -match [regex]::Escape($Group)) {
                $isMatch = $true
            }

            # Match by feature ID (all manual entries for that feature)
            if ($FeatureId -and $featureCol -eq $FeatureId -and $testType -match 'E2E') {
                $isMatch = $true
            }
