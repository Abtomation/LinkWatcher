...
│
├── doc/product-docs/test-audits/   # Test audit reports (7 files)
...
5. **Test audits separated from tests**: Audit reports about test quality live in `doc/product-docs/test-audits/` instead of near the tests they evaluate
...
│
├── audits/                         # [MOVED from doc/product-docs/test-audits/]
...
| `doc/product-docs/test-audits/` | `test/audits/` | Moved (audit reports belong near the tests they evaluate) |
...
5. **Test audits co-located**: Audit reports live near the tests they evaluate, not in a separate product-docs tree
...
| Old | New |
|-----|-----|
| `run_tests.py` (Python, hard-coded) | doc/process-framework/scripts/test/Run-Tests.ps1 (PowerShell, project-agnostic) |
...
- `doc/process-framework/state-tracking/features/4.1.1-test-suite-implementation-state.md` — all `tests/` references

### Test Audit Reports (path updates after move)

- 7 audit reports moving from `doc/product-docs/test-audits/` to `test/audits/`
- References in `test-tracking.md` (audit report paths in ~20 entries)
- References in `documentation-map.md`
...
- Process framework `documentation-map.md` — test document links, test audit links
...
5. Move `doc/product-docs/test-audits/` to `test/audits/`
...
1. Create doc/process-framework/scripts/test/Run-Tests.ps1 — project-agnostic test runner that reads `project-config.json` for module name, test directory, and test categories
2. Delete `run_tests.py`
3. Create `doc/process-framework/guides/guides/03-testing/test-infrastructure-guide.md` — how `test/` connects to the process framework (directory conventions, relationship between `test/automated/`, `test/specifications/`, `test/manual-testing/`, `test/audits/`, `test-registry.yaml`, and `test-tracking.md`)
...
2. Update `documentation-map.md` — test audit links, removed .md files, new guide
...
- **Test Audit (PF-TSK-030)** — Update audit report output location from `doc/product-docs/test-audits/` to `test/audits/`. Update `New-TestAuditReport.ps1` script if it hard-codes the output path.
...
- **Validation scripts** — `Validate-TestTracking.ps1` and `Validate-StateTracking.ps1` may reference `tests/` or `doc/product-docs/test-audits/` paths
