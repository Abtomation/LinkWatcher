...
> **v2.0 Consolidation**: This file replaces 5 former feature states (PF-FEA-026 through PF-FEA-030, old 3.1.1–3.1.5) as part of the 42→9 feature consolidation. See [Feature Consolidation State](../../../process-framework-local/state-tracking/temporary/old/feature-consolidation-state.md).
...
- Configuration loading (→ 0.1.3 Configuration System)
...
- [✓] Dual-formatter logging (colored console + JSON file)
- [✓] Singleton logger factory with thread-local context
- [✓] Colored console output with icons
- [✓] JSON file logging with rotation
- [✓] Runtime filtering without restart
- [✓] LogTimer for performance measurement
- [✓] Statistics tracking and progress reporting
...
- [✅] **PF-TSK-066**: Retrospective Documentation Creation — Tier Assessment (ART-ASS-197), FDD (PD-FDD-025), TDD (PD-TDD-024) created (2026-02-19/20)
- [✅] **Feature Consolidation**: Merged 3.1.1–3.1.5 into unified feature, new state file created (2026-02-21)
...
| -------- | ---- | ------ | -------- | ------------ |
| Tier Assessment (ART-ASS-197) | Tier Assessment | Complete | [ART-ASS-197](../../documentation-tiers/assessments/PD-ASS-197-3-1-1-logging-system.md) | 2026-02-20 |
| FDD (PD-FDD-025) | Functional Design | Complete | [fdd-3-1-1-logging-framework.md](../../functional-design/fdds/fdd-3-1-1-logging-framework.md) | 2026-02-19 |
| TDD (PD-TDD-024) | Technical Design | Complete | [tdd-3-1-1-logging-framework-t2.md](../../technical/tdd/tdd-3-1-1-logging-framework-t2.md) | 2026-02-20 |
| Test Spec (PF-TSP-041) | Test Specification | Complete | [test-spec-3-1-1-logging-system.md](../../../test/specifications/feature-specs/test-spec-3-1-1-logging-system.md) | 2026-02-24 |

### User Documentation

| Document | Type | Status | Location | Last Updated |
| -------- | ---- | ------ | -------- | ------------ |
| User Handbook (PD-UGD-006) | User Guide | ✅ Created | [logging-and-monitoring.md](../../user/handbooks/logging-and-monitoring.md) | 2026-04-16 |
...
| ~~config-examples/advanced-logging-config.yaml~~ | Example Config | All logging features enabled | **Removed** | Deleted (TD048) — unsupported sections, duplicate of other configs |

### Quick Links

- **Main Design**: [TDD (PD-TDD-024)](../../technical/tdd/tdd-3-1-1-logging-framework-t2.md)
- **Logging Guide**: Removed
...
| --------- | ------- | -------------- | ------ | ------- |
| [linkwatcher/logging.py](../../../src/linkwatcher/logging.py) | Core logging module | `get_logger()`, `setup_logging()`, `LogLevel`, `LogTimer`, `with_context()`, singleton factory | Existing | Pre-framework |
| [linkwatcher/logging_config.py](../../../src/linkwatcher/logging_config.py) | Advanced config | Rotating handlers, JSON formatting, runtime filter management, hot-reload | Existing | Pre-framework |
...
| --------- | ---- | -------------- | ------ | ------- |
| [test/automated/unit/test_logging.py](../../../test/automated/unit/3-logging-monitoring/3-0-logging-monitoring/test_logging.py) | Unit | Logger creation, formatting, filtering, timer | Existing | Pre-framework |
| [test/automated/unit/test_advanced_logging.py](../../../test/automated/unit/3-logging-monitoring/3-0-logging-monitoring/test_advanced_logging.py) | Unit | Advanced logging config, rotation, JSON formatting | Existing | Pre-framework |
...
- **All features** — Every module uses `get_logger()` for operational logging
...
| ------- | ------- | ------- | ----- |
| structlog | ≥21.0 | Structured logging with context | Pre-framework |
| colorama | ≥0.4 | Colored console output | Pre-framework |
| (stdlib) | — | `logging`, `threading`, `json`, `time` | Pre-framework |
...
|-----------|-----------|-------------------|
| Observability (OB) | Critical | This IS the observability infrastructure — logging, metrics, filtering |
| Extensibility & Maintainability (EM) | Critical | Configurable logging pipeline, runtime filter management, hot-reload |
| Performance & Scalability (PE) | Critical | Logging on every operation — must not be a bottleneck; rotation prevents disk exhaustion |
| Security & Data Protection (SE) | Relevant | Must not log sensitive data (credentials, tokens, full file contents) |

### Not Applicable

| Dimension | Rationale |
|-----------|-----------|
| Accessibility / UX Compliance (UX) | Console output is not interactive UI |
| Data Integrity (DI) | Logs are append-only — no data consistency concerns |
...
**Decision Made**: `get_logger()` as a singleton factory — same logger instance returned for same name, configuration applied once via `setup_logging()`
...
**Decision Made**: Two formatters — colored text for console (with icons), JSON for file output
...
- Where: `src/linkwatcher/logging_config.py`
...
| ---- | ---- | ------ | ------------------ | -------- | ---------------- | ----------------- | ---------- |
| structlog dependency | Tech Debt | Extra dependency for structured logging | Well-maintained library | Low | N/A | Could be replaced with stdlib-only if needed | — |
...
_No immediate actions required. Test specification PF-TSP-041 has been created (2026-02-24)._
