...
- **Business Goal**: Ensure zero missed references — all project links are discoverable in the database immediately after startup
...
- [✓] `_initial_scan()` walks project tree with directory exclusions
- [✓] File filtering consistent with real-time monitoring rules
- [✓] Relative path normalization before storing in database
- [✓] Progress reporting every 50 files
- [✓] `force_rescan()` for full database refresh
...
| -------- | ---- | ---------------- | --------- | ----- |
| ~~HOW_IT_WORKS.md~~ | Architecture Overview | Initial scan phase, link database population | **File Removed** | Was: startup scan description. Content superseded by FDD PD-FDD-024 and TDD PD-TDD-023 |
...
| --------- | ------- | -------------- | ------ | ------- |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | Implements initial scan logic | `_initial_scan()`, recursive directory walk, progress reporting | COMPLETE | Pre-framework |

### Files Modified by This Feature

_No files modified — initial scan is part of the service module._
...
- **[2.1.1 Parser Framework](./2.1.1-parser-framework-implementation-state.md)** (MAINTAINED)
  ...
  - Impact if unavailable: Initial link discovery would fail — no links would be populated
...
| ------------------------ | ----------------------------- | ------------------------------------------ | --------------------------------------- |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | Scan orchestration            | `_initial_scan()`, `force_rescan()`        | Both methods live in service.py         |
| [linkwatcher/database.py](../../../../src/linkwatcher/database.py) | Storing discovered links     | `add_link()`, `clear()`                    | `clear()` used before rescan            |
| [linkwatcher/parser.py](../../../../src/linkwatcher/parser.py)   | Extracting link references   | `parse_file()`                             | Called per monitored file               |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py)  | Directory filter source      | `handler.ignored_dirs`                     | `dirs[:]` pruned using this set         |
...
| --------- | ------------------------- | ----------------- | ----- |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | Calls scan on startup or force-rescan | `_initial_scan()`, `force_rescan()` | Scan triggered by `start(initial_scan=True)` |
...
2. `os.walk()` with `dirs[:] = [...]` in-place mutation: `os.walk()` reads the `dirs` list in-place — mutating it before advancing prevents descent into pruned directories
...
**Rationale**: The in-place mutation of `dirs` is the canonical Python idiom for `os.walk()` directory pruning. It's both efficient (no wasted traversal) and idiomatic. The filter reuses `handler.ignored_dirs` — the same set used during real-time monitoring — for consistency.
...
**Validation**: Confirmed — `dirs[:] = [d for d in dirs if d not in self.handler.ignored_dirs]` in `_initial_scan()`.
...
2. Optional scan parameter: `start(initial_scan=True)` — caller controls whether to pre-populate the database
...
- Default behavior is correct — new users get a fully populated database at startup
...
**Validation**: Confirmed — `if initial_scan: self._initial_scan()` in `start()`.
...
- Why: Canonical Python idiom — prevents descent into ignored directories without post-filtering
