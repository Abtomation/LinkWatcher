...
- [✓] Observer creation and lifecycle management in `LinkWatcherService.start()`/`stop()`
- [✓] Single recursive watch on project root
- [✓] Clean shutdown with `observer.stop()` + `observer.join()`
- [✓] Cross-platform watchdog backend selection
...
| -------- | ---- | ---------------- | --------- | ----- |
| ~~HOW_IT_WORKS.md~~ | Architecture Overview | File system monitoring setup, watchdog observer configuration | **File Removed** | Was: watchdog integration description. Content superseded by FDD PD-FDD-024 and TDD PD-TDD-023 |
...
| --------- | ------- | -------------- | ------ | ------- |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | Integrates watchdog Observer for monitoring | Observer setup, `schedule()`, `start()`, graceful shutdown with `join()` | COMPLETE | Pre-framework |

### Files Modified by This Feature

_No files modified — watchdog integration is part of the service module._
...
- **[0.1.1 Core Architecture](../0.1.1-core-architecture-implementation-state.md)** (MAINTAINED)
  - Why: `LinkWatcherService` owns the Observer lifecycle — it creates, schedules, starts, stops, and joins the Observer
...
| -------- | ------- | --------------------------------- | ------------- |
| watchdog | ≥2.0    | File system event monitoring      | Pre-framework |
...
| ---------------------- | ------------------------------- | --------------------------------------------------------- | -------------------------- |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py) | Event dispatch target         | `LinkMaintenanceHandler` registered via `.schedule()`     | Handler receives all events |
...
1. Create Observer in `__init__()`: Observer exists as soon as service is constructed — simpler but starts background thread early, cannot restart service cleanly
2. Create Observer in `start()`: Observer is created only when monitoring is activated — allows service to exist without monitoring active, supports future restartability

**Decision Made**: Lazy creation in `start()` — Observer is created each time `start()` is called
...
**Validation**: Confirmed — `Observer()` is instantiated inside `start()`, not `__init__()`.
...
2. Single recursive watch on project root: Simplicity — one `schedule()` call covers the entire project tree
3. Multiple recursive watches on specific subdirectories: Middle ground but arbitrary scope boundaries

**Decision Made**: `observer.schedule(self.handler, str(self.project_root), recursive=True)` — single recursive watch
...
**Validation**: Confirmed — one `observer.schedule()` call in `start()`.
