...
The path utilities feature is a pure functions module (`utils.py`) providing cross-platform path handling capabilities used by the core monitoring and parsing subsystems. It contains no classes — only module-level functions that perform file monitoring eligibility checks, directory filtering, path normalization, and safe file I/O.
...
- `should_monitor_file()` — checks if a file's extension is in the monitored set and not in an ignored directory
- `should_ignore_directory()` — checks if a directory name matches the ignore list
- `safe_file_read()` — encoding-safe file reader with error recovery
- `looks_like_file_path()` — heuristic path-string detector
- `normalize_path()`, `get_relative_path()`, `find_line_number()`, `format_file_size()` — utility functions (present in codebase, no confirmed active callers)
...
- [✓] `should_monitor_file()` — used by `handler.py` for event filtering
- [✓] `should_ignore_directory()` — used by `handler.py` for directory filtering
- [✓] `safe_file_read()` — used by `parsers/base.py` for all file reads
- [✓] `looks_like_file_path()` — used by `parsers/base.py` for link detection
- [✓] `normalize_path()`, `get_relative_path()`, `find_line_number()`, `format_file_size()` — present but no confirmed active callers

### What's In Progress

- [⚙] Retrospective documentation (PF-TSK-065 Analysis, PF-TSK-066 TDD creation pending for Tier 2)

### What's Blocked

- [🚫] Dead code investigation needed: `normalize_path()`, `get_relative_path()`, `find_line_number()`, `format_file_size()` have no confirmed importers. Potential tech debt — may be unused or called indirectly.
...
- [✅] **PF-TSK-064**: Codebase Feature Discovery
  ...
  - **Session Notes**: Identified as Tier 2 feature — non-trivial path handling with Windows-specific logic
...
- [ ] **PF-TSK-066**: Retrospective Documentation Creation
  ...
  - **Note**: Tier 2 feature — TDD creation recommended for path handling logic and Windows compatibility guarantees
...
| -------- | ---- | ---------------- | --------- | ----- |
| ~~HOW_IT_WORKS.md~~ | Architecture Overview | Path normalization, resolution, relative path calculation | **File Removed** | Was: path utility description. Content superseded by FDD PD-FDD-022 and TDD PD-TDD-021 |
...
| --------- | ------- | -------------- | ------ | ------- |
| [linkwatcher/utils.py](../../../../src/linkwatcher/utils.py) | Windows path handling utilities | Path normalization, relative path computation, Windows-specific path handling | COMPLETE | Pre-framework |

### Files Modified by This Feature

_No files modified — utilities are a standalone module._
...
_No internal feature dependencies — pure functions using Python standard library only._
...
- **[2.1.1 Parser Framework](./2.1.1-parser-framework-implementation-state.md)** (MAINTAINED)
  ...
  - Note: All 6 concrete parsers inherit these via `BaseParser` — none import `utils.py` directly
...
_No internal code dependencies — uses only Python standard library (`os`, `pathlib`, `typing`)._

**Reverse Code Dependencies** (files that import this feature):

| Component | How They Use This Feature | Methods/APIs Used | Notes |
| --------- | ------------------------- | ----------------- | ----- |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py) | Event path filtering | `should_monitor_file()`, `should_ignore_directory()` | Only confirmed direct importer in core system |
| [linkwatcher/parsers/base.py](../../../../src/linkwatcher/parsers/base.py) | File reading, path detection | `safe_file_read()`, `looks_like_file_path()` | All 6 concrete parsers inherit via `BaseParser` — none import `utils.py` directly |

> **Note**: `service.py`, `updater.py`, and `database.py` do NOT import `utils.py`. Functions `normalize_path()`, `get_relative_path()`, `find_line_number()`, and `format_file_size()` have no confirmed direct importers — potential dead code.
...
3. Mixin class: Unnecessary coupling — utilities should be usable by any component
...
**Validation**: Confirmed — `utils.py` contains 8 module-level functions, no classes.
...
3. Return empty string: Parser sees "no content" and produces no links — safe, simple

**Decision Made**: `safe_file_read()` catches all read errors and returns empty string

**Rationale**: An empty string is the correct "no content" sentinel for parsers — they will find no links and move on. This prevents cascading failures from individual unreadable files without requiring per-call error handling in every parser.

**Implications**:

- Unreadable files are silently skipped — no warning logged (potential silent failure)
- Parsers require no error handling around file reads
- Binary files or encoding mismatches are safely handled

---

### Decision 3: Potential Dead Code — Four Unconfirmed Functions
...
- `normalize_path()` in `utils.py` and `_normalize_path()` in `database.py` may have diverged — duplication risk
...
| ---- | ---- | ------ | ------------------ | -------- | ---------------- | ----------------- | ---------- |
| 4 potentially dead functions: `normalize_path()`, `get_relative_path()`, `find_line_number()`, `format_file_size()` | Tech Debt | No confirmed direct importers found during PF-TSK-065 analysis | None — functions remain in codebase | Medium | 2h (confirm callers + remove or document) | Confirm no indirect callers exist; remove if dead | TD001 |
| Duplicate `normalize_path()` implementations | Tech Debt | `utils.py` has `normalize_path()` and `database.py` has `_normalize_path()` — possible divergence | Both implementations exist independently | Medium | 1h (compare, consolidate) | Consolidate into single implementation in utils.py; database.py should import it | TD002 |
| `safe_file_read()` silently swallows errors on unreadable files | Tech Debt | Catches all exceptions and returns empty string — unreadable files are skipped without any log entry | None — failures are invisible | High | 1h (add logging) | Add `logger.warning()` or `logger.debug()` call in the exception handler so skipped files are visible in logs | TD003 |
