...
- **User Need**: Visibility into file-watching operations — what was moved, what links were updated, and any errors encountered
...
- [✓] `LinkWatcherLogger` with 5 log levels, structured output via structlog
- [✓] Colored console output (`ColoredFormatter`) with colorama
- [✓] JSON file logging with `RotatingFileHandler` (10MB default, 5 backups)
- [✓] Thread-safe context via `LogContext` with `threading.local()`
- [✓] Performance timing via `PerformanceLogger` and `LogTimer` context manager
- [✓] Domain-specific convenience methods (`file_moved`, `file_deleted`, `links_updated`, `scan_progress`, `operation_stats`)
- [✓] `LogFilter` with component/operation/level/time/pattern filtering
- [✓] `LogMetrics` with thread-safe per-level/component/operation counters
- [✓] `LoggingConfigManager` with JSON/YAML config loading and hot-reload
- [✓] Backward compatibility functions (`log_file_moved`, `log_error`, etc.)
- [✓] `with_context()` decorator for scoped logging context
...
- [✓] **PF-TSK-066**: Documentation Creation — Complete ✅ (FDD PD-FDD-025, TDD PD-TDD-024; 2026-02-19)
...
| -------- | ---- | ------ | -------- | ------------ |
| [FDD PD-FDD-025: Logging Framework](/doc/functional-design/fdds/fdd-3-1-1-logging-framework.md) | FDD | Created | doc/functional-design/fdds/fdd-3-1-1-logging-framework.md | 2026-02-19 |
| [TDD PD-TDD-024: Logging Framework](/doc/technical/tdd/tdd-3-1-1-logging-framework-t2.md) | TDD (Tier 2) | Created | doc/technical/tdd/tdd-3-1-1-logging-framework-t2.md | 2026-02-19 |
...
| --------- | --------- | -------------- | -------- | ---------- |
| [../../../../src/linkwatcher/logging.py](../../../../src/linkwatcher/logging.py) | Logging framework, structured logging | Logger configuration | Existing | N/A |
| [../../../../src/linkwatcher/logging_config.py](../../../../src/linkwatcher/logging_config.py) | Advanced logging configuration | Log formatting, handlers | Existing | N/A |
| ~~config-examples/advanced-logging-config.yaml~~ | Advanced logging config example | Configuration patterns | **Removed** | Deleted (TD048) |
...
- **[0.1.1 Core Architecture](../0.1.1-core-architecture-implementation-state.md)** — service.py uses `get_logger()`, `LogTimer`, `with_context()`
- **[0.1.3 In-Memory Database](./0.1.3-in-memory-database-implementation-state.md)** — database.py uses `get_logger()`
- **[1.1.2 Event Handler](./1.1.2-event-handler-implementation-state.md)** — handler.py uses `get_logger()`, `with_context()`, `LogTimer`
- **[2.1.1 Parser Framework](./2.1.1-parser-framework-implementation-state.md)** — parser.py uses `get_logger()`, `LogTimer`
- **[2.2.1 Link Updater](./2.2.1-link-updater-implementation-state.md)** — updater.py uses `get_logger()`, `LogTimer`, `with_context()`
- **[3.1.2 Colored Console Output](./3.1.2-colored-console-output-implementation-state.md)** — sub-feature within logging.py
- **[3.1.3 Statistics Tracking](./3.1.3-statistics-tracking-implementation-state.md)** — LogMetrics uses logging infrastructure
- **[3.1.4 Progress Reporting](./3.1.4-progress-reporting-implementation-state.md)** — scan_progress and LogTimer in logging.py
- **[3.1.5 Error Reporting](./3.1.5-error-reporting-implementation-state.md)** — error/critical/exception methods in logging.py
...
| [linkwatcher/__init__.py](../../../../src/linkwatcher/__init__.py) | Public API export | Logging API exports | Package-level export |
| [linkwatcher/database.py](../../../../src/linkwatcher/database.py) | Database operation logging | `get_logger()` | Database logging |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py) | Event handling logging | `get_logger()`, `with_context()`, `LogTimer` | Event logging |
| [linkwatcher/parser.py](../../../../src/linkwatcher/parser.py) | Parsing operation logging | `get_logger()`, `LogTimer` | Parser logging |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | Service operation logging | `get_logger()`, `LogTimer`, `with_context()` | Service logging |
| [linkwatcher/updater.py](../../../../src/linkwatcher/updater.py) | Update operation logging | `get_logger()`, `LogTimer`, `with_context()` | Updater logging |
| [linkwatcher/logging_config.py](../../../../src/linkwatcher/logging_config.py) | Advanced logging configuration | Logging configuration functions | Extended logging features |
...
- `structlog.configure()` called in `LinkWatcherLogger.__init__()` — reconfiguration requires re-instantiation
...
**Rationale**: Simple, zero-config for consumers — any module calls `get_logger()` and gets the configured logger. `setup_logging()` allows one-time configuration at startup (e.g., from CLI args in `main.py`).
...
- Testing requires care — singleton state persists across tests unless explicitly reset
...
**Implications**:
- Context is automatically isolated per thread — no cross-thread pollution
