...
The Event Handler (`LinkMaintenanceHandler`) is the central coordinator for real-time file system event processing. It inherits from watchdog's `FileSystemEventHandler` and overrides `on_moved()`, `on_deleted()`, and `on_created()` to implement LinkWatcher's core response logic. `on_modified()` is intentionally not overridden — LinkWatcher tracks path changes, not content changes.

For **file move events**, the handler (1) queries the link database for all references to the old path using multiple path format strategies (exact, relative, backslash, filename-only), (2) calls the updater to rewrite those references in their containing files, and (3) rescans all affected files to rebuild the database. For **directory moves**, it walks the new directory, calculates old→new path mappings for each file, and processes each file's references individually.
...
- [✓] File move handling with multi-format path lookup and deduplication
- [✓] Directory move handling with file-by-file processing
- [✓] Delayed move detection (2-second buffer for delete+create moves)
- [✓] File creation scanning (new files added to database)
- [✓] Statistics tracking (files_moved, files_deleted, files_created, links_updated, errors)
- [✓] `@with_context` logging decorator for structured event logging

### What's In Progress

_Nothing in progress — feature is stable._
...
- [✓] **PF-TSK-064**: Codebase Feature Discovery & Code Assignment
  - **Completed**: 2026-02-18
  - **Key Outputs**: Feature state file (PF-FEA-010) created; code inventory populated in Section 5
  - **Status**: COMPLETE ✅

- [✓] **PF-TSK-065**: Codebase Feature Analysis ← CURRENT SESSION
  - **Completed**: 2026-02-18
...
| -------- | ---- | ------ | -------- | ------------ |
| [FDD PD-FDD-024: Event Handler](/doc/functional-design/fdds/fdd-1-1-2-event-handler.md) | FDD | Created | doc/functional-design/fdds/fdd-1-1-2-event-handler.md | 2026-02-19 |
| [TDD PD-TDD-023: Event Handler](/doc/technical/tdd/tdd-1-1-2-event-handler-t2.md) | TDD (Tier 2) | Created | doc/technical/tdd/tdd-1-1-2-event-handler-t2.md | 2026-02-19 |
...
| -------- | ---- | ---------------- | --------- | ----- |
| ~~HOW_IT_WORKS.md~~ | Architecture Overview | Move/delete event detection, event processing pipeline | **File Removed** | Was: event handler architecture. Content superseded by FDD PD-FDD-024 and TDD PD-TDD-023 |
...
| --------- | ------- | -------------- | ------ | ------- |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py) | File system event handler | `LinkWatcherHandler`, `on_moved()`, `on_created()`, `on_deleted()`, `on_modified()` | COMPLETE | Pre-framework |
...
_No files modified — handler is a standalone component._
...
| --------- | ------- | ---------------------------------------------------- | ------------- |
| watchdog  | ≥2.0    | `FileSystemEventHandler` base class, event types     | Pre-framework |
...
| ------------------------ | ----------------------------- | ---------------------------------------------------------- | ---------------------------------- |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py) | All event handler logic       | `on_moved`, `on_deleted`, `on_created`, move detection     | Primary file for this feature      |
| [linkwatcher/database.py](../../../../src/linkwatcher/database.py) | Link lookups                 | `get_references_to_file()`, `remove_links_for_file()`      | Database queried per event         |
| [linkwatcher/updater.py](../../../../src/linkwatcher/updater.py)  | Writing link updates         | `update_references()`                                      | Final step of event processing     |
| [linkwatcher/parser.py](../../../../src/linkwatcher/parser.py)   | Post-move rescan             | `parse_file()`                                             | Rebuilds database after move       |
| [linkwatcher/utils.py](../../../../src/linkwatcher/utils.py)    | File/dir filtering           | `should_monitor_file()`, `should_ignore_directory()`       | All events pass through this check |
...
| --------- | ------------------------- | ----------------- | ----- |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | Instantiates handler and registers with Observer | `LinkMaintenanceHandler()`, passed to `observer.schedule()` | Service owns the handler instance |
...
- The timer callback and `on_created` handler can race — lock required

**Validation**: Confirmed — `pending_deletes` dict and `threading.Timer(2.0, self._execute_delete, [file_path])` in `_handle_file_deleted()`.
...
1. Override `on_modified` to detect when links change in file content: Would catch manual link edits but triggers on every keystroke — very noisy
2. Not overriding `on_modified`: Only structural events (moves, deletes, creates) trigger link maintenance — consistent with the system's purpose
...
- Performance is significantly better — no per-keystroke processing

**Validation**: Confirmed — `LinkMaintenanceHandler` only defines `on_moved`, `on_deleted`, `on_created`.
...
2. Deduplication by file path only: Too coarse — a file might have multiple links to the same target
...
**Validation**: Confirmed in `_handle_file_moved()` — `seen` set initialized per event with 4-tuple keys.
...
- Pattern: [e.g., Observer, Strategy, Facade]
- Why: [Reason]
- Where: [Components]
...
- What: Link updates silently fail — references are not updated after file moves
...
- Analysis of Decision 3 (4-tuple deduplication) → Deduplication itself works correctly even with stale line numbers (same stale values from same database entries, so duplicates still match)
- Analysis of `_update_file_references()` in `updater.py` → Confirmed: the updater uses `ref.line_number - 1` as a direct line index (line 120), which points to the wrong line when content has shifted
- Analysis of `_replace_in_line()` → Confirmed: the method does text matching before replacing, so it won't corrupt data — it just silently fails to update
