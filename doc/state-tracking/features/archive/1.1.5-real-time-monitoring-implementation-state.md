...
Multiple startup scripts support different operating systems and launch contexts. The PowerShell background script (`start_linkwatcher_background.ps1`) is the recommended launcher for development use — it starts LinkWatcher as a background process and records its PID, allowing the developer to continue using the terminal.

### Business Value

- **User Need**: File watching must run continuously in the background without blocking the developer's workflow or terminal
- **Business Goal**: Zero-maintenance link integrity — developers move files freely and LinkWatcher handles all reference updates automatically
...
- Signal handling for graceful shutdown (SIGINT/SIGTERM → `self.running = False`)
- Observer start/stop/join lifecycle management
- Multiple startup scripts (PS1 background, BAT, PS1 interactive, SH, Python)

**Out of Scope**:

- Event processing (feature 1.1.2 — Event Handler)
- Link database management (feature 0.1.3 — In-Memory Database)
...
- [✓] Main monitoring loop with `self.running` flag
- [✓] Signal handlers (SIGINT, SIGTERM) for graceful shutdown
- [✓] Observer lifecycle: start, stop, join
- [✓] Background PowerShell startup script with PID tracking
- [✓] Cross-platform startup scripts (bat, ps1, sh, py)
...
| -------- | ---- | ---------------- | --------- | ----- |
| ~~HOW_IT_WORKS.md~~ | Architecture Overview | Real-time monitoring phase, continuous watching | **File Removed** | Was: monitoring description. Content superseded by FDD PD-FDD-024 and TDD PD-TDD-023 |
...
| --------- | ------- | -------------- | ------ | ------- |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | Main monitoring service orchestration | `start()`, `stop()`, Observer lifecycle management | COMPLETE | Pre-framework |
...
_No files modified — monitoring is a standalone orchestration concern._
...
- **[0.1.1 Core Architecture](../0.1.1-core-architecture-implementation-state.md)** (MAINTAINED)
  - Why: `LinkWatcherService` owns the monitoring loop — `start()` runs the loop, `stop()` terminates it, `_signal_handler()` handles shutdown signals
...
- **[1.1.2 Event Handler](./1.1.2-event-handler-implementation-state.md)** (MAINTAINED)
  - Why: Event handler processes all watchdog events during the monitoring session — without it, events are delivered but not acted upon
  - Impact if unavailable: No link maintenance during monitoring

**Other Features Depend On This**:

_No other features depend on real-time-monitoring — it is the top-level activation feature that orchestrates all file-watching components._
...
| ------------------------ | -------------------------- | --------------------------------------------------- | -------------------------------- |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | Monitoring loop host       | `start()`, `stop()`, `_signal_handler()`            | All loop logic in service.py     |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py) | Event handling             | `LinkMaintenanceHandler` instantiation              | Created by service for Observer  |
| [linkwatcher/database.py](../../../../src/linkwatcher/database.py) | Link storage              | `LinkDatabase` passed to handler                    | Persistence during monitoring    |
| [linkwatcher/parser.py](../../../../src/linkwatcher/parser.py)  | Link parsing               | `LinkParser` passed to handler                      | Link extraction during events    |
| [linkwatcher/updater.py](../../../../src/linkwatcher/updater.py) | Link updating              | `LinkUpdater` passed to handler                     | Updates links on file moves      |
| [linkwatcher/logging.py](../../../../src/linkwatcher/logging.py) | Logging                    | `get_logger()`, logger functions                    | Runtime logging                  |
...
1. `observer.join()`: Blocks until Observer stops — but then there's no way to set a flag for graceful shutdown from a signal handler
2. `while True: time.sleep(1)`: Simple blocking poll — signal handler can set a flag to break the loop
...
**Rationale**: The polling loop is the simplest mechanism that allows both "keep alive while observer runs" and "signal-based graceful shutdown." The 1-second sleep granularity is acceptable — no need for sub-second shutdown response.
...
**Validation**: Confirmed — `while self.running: time.sleep(1)` in `start()`, inside a `try/finally` block.
...
**Rationale**: `finally` ensures `stop()` always runs — even if an unhandled exception propagates out of the loop. This prevents the Observer background thread from being orphaned.

**Implications**:

- Observer is always properly stopped and joined, even on crash
- `stop()` may be called twice in normal shutdown (once from signal handler path, once from finally) — `stop()` must be idempotent
- Clean process exit even in error scenarios

**Validation**: Confirmed — `try/finally` wrapping in `start()`, `stop()` guards against `observer` being `None`.
...
**Validation**: Confirmed — 5 startup scripts in `LinkWatcher/` directory plus library API via `service.py`.
...
- Pattern: `self.running` flag + signal handlers → `try/finally` cleanup
