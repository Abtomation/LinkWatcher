...
- [✓] `backup_enabled` flag on `LinkUpdater` (default `True`)
- [✓] `shutil.copy2()` creating `.linkwatcher.bak` before each write
- [✓] Graceful failure: warning logged, update continues on backup error
- [✓] `set_backup_enabled()` programmatic API
...
- [✓] **PF-TSK-064**: Codebase Feature Discovery
  ...
  - **Key Outputs**: Feature identified, code assigned to `src/linkwatcher/updater.py` (as backup logic in `_write_file_safely()`)
...
| -------- | ---- | ---------------- | --------- | ----- |
| ~~HOW_IT_WORKS.md~~ | Architecture Overview | Backup creation before file modification, safety mechanism | **File Removed** | Was: backup system description. Content superseded by FDD PD-FDD-027 and TDD PD-TDD-026 |
...
| --------- | ------------ | -------- | -------- | ---------- |
| [../../../../src/linkwatcher/updater.py](../../../../src/linkwatcher/updater.py) | Backup creation before updates | Safety mechanism | Enables rollback | N/A |
...
| --------- | -------- | ----------------- | ----- |
| [linkwatcher/updater.py](../../../../src/linkwatcher/updater.py) | Implementation location | `backup_enabled` flag, `set_backup_enabled()`, `_write_file_safely()` backup step | Inline in LinkUpdater |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | Pass-through API | `set_backup_enabled()` → `self._updater.set_backup_enabled()` | Service layer forwarding |
| [linkwatcher/logging.py](../../../../src/linkwatcher/logging.py) | Logging | `get_logger()`, `logger.info()` | Log backup operations |
| [linkwatcher/config/settings.py](../../../../src/linkwatcher/config/settings.py) | Configuration | `LinkWatcherConfig`, `create_backups` attribute | Backup enabled flag source |
...
| --------- | ------------------------- | ----------------- | ----- |
| This feature is implemented inside `_write_file_safely()` in `src/linkwatcher/updater.py`. All reverse dependencies of [link-updater (2.2.1)](./2.2.1-link-updater-implementation-state.md) also consume this feature indirectly. | | | |
...
1. Abort update on backup failure: Data safety priority — but user loses the benefit of the link update
...
**Date**: 2026-02-18 (discovered)
**Context**: The backup must capture the pre-update state of the file. If taken after the atomic write, the backup would contain the already-updated content — useless as a recovery point.

**Options Considered**:

1. Backup after write: Captures post-update state; useless for recovery
2. Backup before write (chosen): `shutil.copy2()` → tempfile write → `shutil.move()`; backup is always pre-update
...
- Where: `_write_file_safely()` — backup step precedes atomic write step
