...
- [✓] Group-by-file reference processing with bottom-to-top sort
- [✓] Link-type dispatch for precise line replacement
- [✓] Atomic write via temp file + shutil.move
- [✓] Statistics tracking (files_updated, references_updated, errors)
- [✓] dry_run and backup_enabled safety flags with programmatic APIs
...
- [✓] **PF-TSK-064**: Codebase Feature Discovery
  ...
  - **Key Outputs**: Feature identified, code assigned to `src/linkwatcher/updater.py`
...
| -------- | ---- | ---------------- | --------- | ----- |
| ~~HOW_IT_WORKS.md~~ | Architecture Overview | Link update process, atomic file updates, safety mechanisms | **File Removed** | Was: link updater architecture. Content superseded by FDD PD-FDD-027 and TDD PD-TDD-026 |
...
| --------- | --------- | -------------- | -------- | ---------- |
| [../../../../src/linkwatcher/updater.py](../../../../src/linkwatcher/updater.py) | Atomic link update with safety | LinkUpdater class | Existing | N/A |
...
| --------- | -------- | ----------------- | ----- |
| [linkwatcher/models.py](../../../../src/linkwatcher/models.py) | Input data type | `LinkReference` (line_number, column_start, link_type, link_target) | Read-only |
| [linkwatcher/logging.py](../../../../src/linkwatcher/logging.py) | Structured logging | `get_logger()`, `LogTimer`, `with_context()` | Update progress and timing |
...
| --------- | ------------------------- | ----------------- | ----- |
| [linkwatcher/__init__.py](../../../../src/linkwatcher/__init__.py) | Public API export | `from .updater import LinkUpdater` | Package-level export |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | Updater instantiation | `LinkUpdater(project_root)`, `.set_dry_run()` | Service-level updater management |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py) | Link update execution | `LinkUpdater`, `.update_references()` | Event-driven link updates |
...
1. Top-to-bottom order with position recalculation after each edit: Complex; error-prone; O(n²) for n references per file
...
3. Line-by-line reconstruction: Read all lines, modify in memory, write once — viable but requires holding full file in memory with line-indexed references
...
- All references in a file are collected first, then sorted, then applied — no streaming update
...
**Rationale**: `shutil.move()` on the same filesystem performs an atomic rename. At any point, either the old or new file exists — never a partial state. The temp file is in the same directory to ensure same-filesystem move.
...
2. Column-position–based replacement (chosen as `_replace_at_position()`): Uses stored `column_start` to target the exact character range
...
- Pattern: Strategy (link_type → replacement method)
