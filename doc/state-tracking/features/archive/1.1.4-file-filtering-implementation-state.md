...
- Pattern-based exclusion (e.g., glob patterns — not implemented)
...
- [✓] Extension-based file filtering via `monitored_extensions` set in handler
- [✓] Directory exclusion via `ignored_dirs` set in handler
- [✓] Delegation to `utils.should_monitor_file()` for filtering predicate
- [✓] Runtime extension addition via `service.add_parser()`
...
- [🚫] Config integration gap: `LinkWatcherConfig.monitored_extensions` and `.ignored_directories` fields exist but are not used by the handler. Hard-coded lists in handler cannot be customized via config file. Technical debt.
...
- [ ] **PF-TSK-066**: Retrospective Documentation Creation
  - **Tier**: Tier 1 — no additional documentation required (no TDD, FDD, or Test Spec needed)
  - **Status**: PENDING
...
| -------- | ---- | ---------------- | --------- | ----- |
| ~~HOW_IT_WORKS.md~~ | Architecture Overview | File type detection, extension-based filtering | **File Removed** | Was: file filtering description. Content superseded by FDD PD-FDD-024 and TDD PD-TDD-023 |
...
| --------- | ------- | -------------- | ------ | ------- |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py) | Implements file extension and directory filtering | `monitored_extensions` set, `ignored_dirs` set, extension check in event handlers | COMPLETE | Pre-framework |

### Files Modified by This Feature

_No files modified — file filtering is part of the handler module._
...
**Note — Config Integration Gap (Technical Debt)**:
...
- **[1.1.2 Event Handler](./1.1.2-event-handler-implementation-state.md)** (MAINTAINED)
  - Why: All event handler logic (on_moved, on_deleted, on_created) gates processing through `_should_monitor_file()` — this feature's check is the entry gate for all event processing
...
| ------------------------ | ----------------------------- | ------------------------------------------------------------- | ---------------------------------------- |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py) | Filter sets and check method  | `monitored_extensions`, `ignored_dirs`, `_should_monitor_file()` | Filter data lives in handler constructor |
| [linkwatcher/utils.py](../../../../src/linkwatcher/utils.py)   | Filter predicate logic        | `should_monitor_file()`, `should_ignore_directory()`          | Actual filter logic delegated here       |
| [linkwatcher/config/settings.py](../../../../src/linkwatcher/config/settings.py) | Configuration fields   | `LinkWatcherConfig`, `monitored_extensions`, `ignored_directories` | Config fields exist but are NOT wired up (tech debt) |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | Runtime extension addition    | `add_parser()` updates `handler.monitored_extensions`         | Only runtime modification point          |
...
| --------- | ------------------------- | ----------------- | ----- |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py) | Event handlers gate processing through filter | `_should_monitor_file()` | Entry gate for all event processing |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | `_initial_scan()` prunes directories via filter | `handler.ignored_dirs` | Shared filter sets for scan + monitoring |
...
1. Read from `LinkWatcherConfig`: User-configurable via config file — but requires config to be passed to handler and all config fields to be wired up
2. Hard-code in handler `__init__()`: Simpler implementation, no config dependency in handler — but not user-configurable
3. Separate filter class: More abstraction but adds indirection for a relatively simple concern

**Decision Made**: Hard-coded sets in `LinkMaintenanceHandler.__init__()` — `monitored_extensions` and `ignored_dirs` are defined directly in the constructor

**Rationale**: This was likely the original implementation approach — hard-code reasonable defaults that cover the typical use case. The `LinkWatcherConfig` fields (`monitored_extensions`, `ignored_directories`) were added later as planned config integration but the handler was never updated to read them. This creates a technical debt gap.

**Implications**:

- Users cannot customize file filtering via config file despite config fields existing for this purpose
- `service.add_parser()` provides the only runtime extension point — limited to adding extensions, not removing or changing directory exclusions
...
**Validation**: Confirmed — `monitored_extensions` and `ignored_dirs` sets defined in `LinkMaintenanceHandler.__init__()`, not read from config.
...
1. Extension-only filtering: Ignores location — would scan files inside `node_modules` if they have monitored extensions
2. Directory-only filtering: Ignores file type — would attempt to parse binary files in monitored directories
3. Both dimensions (extension + directory): Full coverage — type-based inclusion AND location-based exclusion
...
**Validation**: Confirmed — `_should_monitor_file()` checks both extension and path against both sets.
...
| ------ | --------- | -------- | ------------------ | ---------- | ---------------- | ----------------- | ---------- |
| Config fields ignored by handler — `LinkWatcherConfig.monitored_extensions` and `.ignored_directories` exist but `LinkMaintenanceHandler.__init__()` hard-codes its own lists | Tech Debt | Original implementation hard-coded defaults; config fields added later but never wired up | `service.add_parser()` allows runtime extension addition only | Medium | 3h | Handler `__init__()` should accept config and read filter criteria from it, falling back to hard-coded defaults | TD004 |
