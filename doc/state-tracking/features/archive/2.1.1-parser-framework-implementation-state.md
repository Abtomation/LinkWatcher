...
The Parser Framework provides the `LinkParser` facade class that orchestrates file reference extraction across all supported file types. It maintains a dispatch dictionary mapping file extensions to pre-instantiated parser objects: `.md` → `MarkdownParser`, `.yaml`/`.yml` → `YamlParser`, `.json` → `JsonParser`, `.dart` → `DartParser`, `.py` → `PythonParser`. A `GenericParser` instance handles all extensions not present in the dispatch map.
...
- [✓] Extension dispatch to 6 specialized parsers
- [✓] GenericParser fallback for unknown extensions
- [✓] Runtime `add_parser()`/`remove_parser()` API
- [✓] `get_supported_extensions()` query
- [✓] LogTimer wrapping for performance tracking
...
- [✓] **PF-TSK-064**: Codebase Feature Discovery
  ...
  - **Key Outputs**: Feature identified, code assigned to `src/linkwatcher/parser.py`
...
| -------- | ---- | ------ | -------- | ------------ |
| [FDD PD-FDD-026: Parser Framework](/doc/functional-design/fdds/fdd-2-1-1-parser-framework.md) | FDD | Created | doc/functional-design/fdds/fdd-2-1-1-parser-framework.md | 2026-02-19 |
| [TDD PD-TDD-025: Parser Framework](/doc/technical/tdd/tdd-2-1-1-parser-framework-t2.md) | TDD (Tier 2) | Created | doc/technical/tdd/tdd-2-1-1-parser-framework-t2.md | 2026-02-19 |
...
| -------- | ---- | ---------------- | --------- | ----- |
| ~~HOW_IT_WORKS.md~~ | Architecture Overview | Parser system overview, base parser design, parser registry | **File Removed** | Was: parser framework architecture. Content superseded by FDD PD-FDD-026 and TDD PD-TDD-025 |
...
| --------- | --------- | -------------- | -------- | ---------- |
| [../../../../src/linkwatcher/parser.py](../../../../src/linkwatcher/parser.py) | LinkParser facade orchestrating all parsers | LinkParser class | Existing | N/A |
| [../../../../src/linkwatcher/parsers/__init__.py](../../../../src/linkwatcher/parsers/__init__.py) | Parsers package init | Package exports | Existing | N/A |
| [../../../../src/linkwatcher/parsers/base.py](../../../../src/linkwatcher/parsers/base.py) | BaseParser abstract class | Abstract parser interface | Existing | N/A |

### Files Modified by This Feature

N/A — Core framework feature
...
- **[PF-FEA-026: logging-framework](./3.1.1-logging-framework-implementation-state.md)** (MAINTAINED)
  ...
  - Status: MAINTAINED
  - Impact if unavailable: Parse timing lost; minor — parsing still functions
...
_None — operates on local files only_
...
| --------- | -------- | ----------------- | ----- |
| [linkwatcher/models.py](../../../../src/linkwatcher/models.py) | Return type annotation | `LinkReference` | Type contract for all parser returns |
| [linkwatcher/logging.py](../../../../src/linkwatcher/logging.py) | Performance tracking | `get_logger()`, `LogTimer` | Wraps parse_file() call |
| [linkwatcher/utils.py](../../../../src/linkwatcher/utils.py) | File reading and path detection | `safe_file_read()`, `looks_like_file_path()` | Imported by `parsers/base.py` |
| [linkwatcher/parsers/markdown.py](../../../../src/linkwatcher/parsers/markdown.py) | Markdown parsing | `MarkdownParser` | .md/.markdown handler |
| [linkwatcher/parsers/yaml_parser.py](../../../../src/linkwatcher/parsers/yaml_parser.py) | YAML parsing | `YamlParser` | .yaml/.yml handler |
| [linkwatcher/parsers/json_parser.py](../../../../src/linkwatcher/parsers/json_parser.py) | JSON parsing | `JsonParser` | .json handler |
| [linkwatcher/parsers/python.py](../../../../src/linkwatcher/parsers/python.py) | Python import parsing | `PythonParser` | .py handler |
| [linkwatcher/parsers/dart.py](../../../../src/linkwatcher/parsers/dart.py) | Dart import parsing | `DartParser` | .dart handler |
| [linkwatcher/parsers/generic.py](../../../../src/linkwatcher/parsers/generic.py) | Generic fallback parsing | `GenericParser` | Fallback handler |
...
| --------- | ------------------------- | ----------------- | ----- |
| [linkwatcher/__init__.py](../../../../src/linkwatcher/__init__.py) | Public API export | `from .parser import LinkParser` | Package-level export |
| [linkwatcher/service.py](../../../../src/linkwatcher/service.py) | File parsing in service | `LinkParser()`, `.parse_file()` | Initial scan parsing |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py) | Event-driven parsing | `LinkParser`, `.parse_file()` | File event parsing |
...
1. Direct if/elif dispatch in parse_file(): Simple but non-extensible — every new type requires modifying the method
...
3. Factory + on-demand instantiation: Creates new parser instances per call — wasteful for parsers with pre-compiled regex
...
- Pattern: Registry dict (extension → parser instance)
