...
- **User Need**: Provides a shared vocabulary for cross-reference data — every component speaks the same typed language when passing link and event information.
...
- [✓] `LinkReference` dataclass with 7 fields: `file_path`, `line_number`, `column_start`, `column_end`, `link_text`, `link_target`, `link_type`
- [✓] `FileOperation` dataclass with 4 fields: `operation_type`, `old_path`, `new_path`, `timestamp`
- [✓] Standard `@dataclass` decorator providing `__init__`, `__repr__`, `__eq__` for both
- [✓] Type annotations using `Optional`, `str`, `int`, `datetime` from standard library
- [✓] Zero external package dependencies

### What's In Progress

- [⚙] Retrospective documentation (PF-TSK-065 Analysis, PF-TSK-066 not required for Tier 1)
...
- [✅] **PF-TSK-064**: Codebase Feature Discovery
  ...
  - **Session Notes**: Identified as Tier 1 feature — pure data containers, no architectural complexity
...
- [ ] **PF-TSK-066**: Retrospective Documentation Creation
  ...
  - **Note**: Tier 1 feature — no TDD or FDD required per documentation tier guidelines
...
| -------- | ---- | ---------------- | --------- | ----- |
| ~~HOW_IT_WORKS.md~~ | Architecture Overview | LinkReference model, data structures, database schema | **File Removed** | Was: core data model description. Content superseded by FDD PD-FDD-022 and TDD PD-TDD-021 |
...
| --------- | ------- | -------------- | ------ | ------- |
| [linkwatcher/models.py](../../../../src/linkwatcher/models.py) | Core data models for the entire system | `LinkReference` (file link representation), `FileOperation` (move/rename events) | COMPLETE | Pre-framework |

### Files Modified by This Feature

_No files modified — data models are self-contained._
...
_No database schema changes — in-memory only._
...
_No feature dependencies — pure Python standard library only._
...
_No internal code dependencies — uses only Python built-in libraries (`dataclasses`, `datetime`, `typing`)._

**Reverse Code Dependencies** (files that import this feature):

| Component | How They Use This Feature | Methods/APIs Used | Notes |
| --------- | ------------------------- | ----------------- | ----- |
| [linkwatcher/__init__.py](../../../../src/linkwatcher/__init__.py) | Public API export | `from .models import LinkReference, FileOperation` | Package-level export |
| [linkwatcher/database.py](../../../../src/linkwatcher/database.py) | Link storage | `LinkReference` | Stores link references |
| [linkwatcher/handler.py](../../../../src/linkwatcher/handler.py) | Event processing | `LinkReference`, `FileOperation` | Event and link handling |
| [linkwatcher/parser.py](../../../../src/linkwatcher/parser.py) | Link extraction | `LinkReference` | Returns parsed links |
| [linkwatcher/updater.py](../../../../src/linkwatcher/updater.py) | Link updates | `LinkReference` | Updates link references |
| [linkwatcher/parsers/base.py](../../../../src/linkwatcher/parsers/base.py) | Base parser | `LinkReference` | Abstract parser interface |
| [linkwatcher/parsers/markdown.py](../../../../src/linkwatcher/parsers/markdown.py) | Markdown parsing | `LinkReference` | Creates link references |
| [linkwatcher/parsers/yaml_parser.py](../../../../src/linkwatcher/parsers/yaml_parser.py) | YAML parsing | `LinkReference` | Creates link references |
| [linkwatcher/parsers/json_parser.py](../../../../src/linkwatcher/parsers/json_parser.py) | JSON parsing | `LinkReference` | Creates link references |
| [linkwatcher/parsers/python.py](../../../../src/linkwatcher/parsers/python.py) | Python parsing | `LinkReference` | Creates link references |
| [linkwatcher/parsers/dart.py](../../../../src/linkwatcher/parsers/dart.py) | Dart parsing | `LinkReference` | Creates link references |
| [linkwatcher/parsers/generic.py](../../../../src/linkwatcher/parsers/generic.py) | Generic parsing | `LinkReference` | Creates link references |
...
- No serialization built-in — if persistence is needed later, a separate serializer must be added
...
- Where: `src/linkwatcher/models.py` (`LinkReference`, `FileOperation`)
