...
| TD268 | No column-span verification at the position-replacement choke point: updater._replace_at_position slices line[column_start:column_end] and substitutes without ever checking the slice equals ref.link_target. The only upstream guard is line-level containment in _apply_replacements, which a column-drift defect passes. This is the amplifier that turned PD-BUG-118 Defect A (parser column drift) into silent corruption of 91 files; the fix repaired the parser and left the amplifier unguarded, so any future column-drift defect in any of the 7 parsers corrupts content the same silent way. | DI OB | src/linkwatcher/updater.py::_replace_at_position (near line 601), src/linkwatcher/updater.py::_apply_replacements (near line 365) | 2026-08-17 | High | S | Open | - | - | - | Discovered during Code Review (PF-TSK-005) of the PD-BUG-118 fix, 2026-08-17. Suggested shape: before substituting, verify line[column_start:column_end] equals ref.link_target (or the quote-wrapped form the method already special-cases); on mismatch fall through to the existing bounded single-occurrence fallback and emit a warning plus the _ambiguous_skip_count increment, mirroring the ambiguous_fallback_replacement_skipped path already in the same method. Defense in depth only - no known column drift remains after PD-BUG-118 (cross-parser sweep at review time found 0 drift across all 7 parsers). |
| TD269 | PD-BUG-119 regression test test_registration_is_not_lost_to_a_concurrent_apply no longer discriminates: it hooks time.monotonic() to enter the window between setdefault and the store, but the fix hoisted that call above the lock, so post-fix the hook fires before the critical section. Re-splitting setdefault and the store across two lock acquisitions leaves the suite green. | DI | test/automated/unit/2-link-parsing-update/test_movememorythreadsafety.py (TestPendingRecalcLostUpdate) | 2026-08-17 | Low | S | Open | - | - | - | Filed by Code Review (PF-TSK-005) of the PD-BUG-119 fix, 2026-08-17. Mutation-verified against the real test file, three runs: (a) true pre-fix body restored - 3 failed, so the harness discriminates; (b) prune moved back outside the lock - 3 failed, so the two prune-collision pins and the stress backstop ARE load-bearing; (c) setdefault and store re-split across two acquisitions, keeping the hoisted now - 5 passed, so this one test does not catch the defect its own docstring names. The weakness is isolated: 4 of the 5 tests hold up. Fix: hook something inside the critical section (for example a dict subclass whose setdefault fires the hook) rather than a call the implementation is free to relocate. The same stale mechanism description appears in the TDD thread-safety note and in the test-spec Move-Memory Thread Safety section (both say the hook sits between setdefault and the store) and should be corrected with it. The file has not yet been through Test Audit (PF-TSK-030), which may also catch this. |
| TD270 | utils.py AI Context inventory does not list the shared apply_trailing_separator_style or its two callers, so the module docstring under-reports a cross-module function that both recalculation paths depend on. Every other shared function in that module is listed there with its primary callers. | DA | src/linkwatcher/utils.py (module docstring, High-traffic functions and primary callers list) | 2026-08-18 | Low | S | Open | - | - | - | Found during Code Review (PF-TSK-005) of the PD-BUG-120 fix, 2026-08-18. The function was promoted from a PathResolver static method to shared utils by that fix and is now called from path_resolver.py and reference_lookup.py. |
| TD271 | Control Panel view-layer fixes have no automated regression protection: the widget modules sit at 16-20 percent coverage by the test spec no-widget-instantiation rule, so the CR-2 _fail_closed guard (last defense against cross-project config overwrite), CR-6 keyboard navigation, CR-15 and the CR-16 status bar are pinned only by one-off scripted real-Tk runs that were not retained as runnable artifacts. | TST,UX,DI | src/linkwatcher/linkwatcher_control_panel/views/ | 2026-08-18 | Medium | Medium | Open | - | - | - | Surfaced by Code Review (PF-TSK-005) of feature 7.1.1 gap-closure delta, 2026-08-18, as finding CR-25. Pre-existing architectural characteristic, not introduced by the delta - filed as debt rather than held against it. Searched for retained real-Tk verification scripts: none exist. Candidate approach: a headless-driveable seam or a retained scripted Tk harness run outside the pytest suite, so the Critical-dimension guards regain a regression pin. |
