...
- [x] `test/automated/unit/3-logging-monitoring/3-0-logging-monitoring/test_logging.py` — Added `test_disabled_skips_logging` (success path) and `test_disabled_swallows_exception_path` (exception path) to `TestLogTimer`. Both verify that `enabled=False` skips `start_timer`/`end_timer` calls and emits no debug/error logs while still letting exceptions propagate naturally.
