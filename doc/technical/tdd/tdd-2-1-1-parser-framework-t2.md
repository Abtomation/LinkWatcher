...
| `src/linkwatcher/parsers/base.py` | `BaseParser` abstract class — extension contract. Also hosts shared match-hygiene helpers: `_classify_path()`, `_walk_structured_data()`, and `_trim_trailing_punctuation()` (PD-BUG-118) |
| `src/linkwatcher/parsers/__init__.py` | Package init — exports all parser classes |
| `src/linkwatcher/parsers/markdown.py` | `MarkdownParser` — `.md` files. Extracts `[text](path)` links, `[label]: path` reference definitions, `<a href>` tags, quoted file/directory paths (`"path"`, `'path"`, `` `path` ``), and standalone file references. Backtick-delimited paths are treated as quoted strings for both file and directory detection. Bare-path and @-prefix targets trim trailing sentence punctuation via the shared `BaseParser._trim_trailing_punctuation` (PD-BUG-118) — the post-match form of the trailing-boundary lookahead the standalone pattern already carries (PD-BUG-080). |
...
| `src/linkwatcher/parsers/python.py` | `PythonParser` — `.py` files. Extracts `import`/`from` statements (stdlib filtered via `sys.stdlib_module_names`), quoted string literals (file paths via `QUOTED_PATH_PATTERN`, directory paths via `QUOTED_DIR_PATTERN`), file paths in `#` comments, and bare file/directory paths inside triple-quoted docstrings (`"""`/`'''`) via docstring state tracking (PD-BUG-062). Docstring content is located as **line-absolute spans** — the state machine records `(start, end)` offsets into the physical line and each match adds its span offset — so recorded columns are correct on lines that themselves contain the triple quotes (PD-BUG-118). Trailing sentence punctuation is trimmed from bare-path targets via the shared `BaseParser._trim_trailing_punctuation` (PD-BUG-118). |
