...
~~`scripts/setup_cicd.py`~~ — Removed (PF-IMP-148). Local CI environment bootstrapping is handled by `dev.bat dev-setup` which installs dependencies and pre-commit hooks.
...
- `dev.bat pre-commit` — installs pre-commit hooks
...
| ~~`scripts/setup_cicd.py`~~ | ~~Local CI environment bootstrapping~~ | Removed (PF-IMP-148) — use `dev.bat dev-setup` |
