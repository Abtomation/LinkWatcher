...
| 2.2.1 | Link Updating | `LinkUpdater.update_references()` groups refs by source file and performs `NamedTemporaryFile` + `shutil.move()` atomic writes per file — so partial bursts on disk are impossible even if the process is killed mid-burst; stale detection (line-out-of-bounds / expected target not on line) converts "move N+1 invalidated line numbers move N recorded" races into `UpdateResult.STALE` + `stats["stale_files"]`, which the handler recovers via `retry_stale_references()`. `ReferenceLookup._move_memory_lock` (PD-BUG-119) serializes the simultaneous-move memory (`_recent_moves`, `_pending_recalcs`) between the observer thread and the `DirectoryMoveDetector` worker; it is held only for the dict operations, never across file I/O or across the `apply_pending_recalcs` repair callback that re-enters the guarded methods |
...
5. **`LinkMaintenanceHandler._handle_file_moved(event)`** ([handler.py:336](src/linkwatcher/handler.py#L336)) runs the full WF-001 pipeline for the single move.
   ...
   - **Cross-move link repair** (PD-BUG-114, 2026-08-10): a burst-specific hazard the serialization property alone does not cover — when moved file A links to moved file B, **both** files vacate their old disk locations before either event is processed, so A's outgoing-link recalculation finds B's old path missing (the PD-BUG-033 existence guard) and B's own event can no longer reach the stale link. `_handle_file_moved` therefore calls `ReferenceLookup.record_move()` up front (a 300s-TTL move memory the guard consults to rewrite links to targets that already have a processed move) and `apply_pending_recalcs()` after processing (re-running the recalc for links an earlier event's guard skipped, now that this move explains them). Both event orders converge on the repaired link; see the TDD's "Simultaneous-Move Repair" section ([PD-TDD-026](../tdd/tdd-2-2-1-link-updater-t2.md)).
