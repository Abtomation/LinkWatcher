...
**Entry point**: User invokes `python main.py [options]` (directly or via `process-framework/tools/linkWatcher/start_linkwatcher_background.ps1`, which wraps the same command). This begins the `main()` function in [main.py](main.py).

**Exit point**: The `watchdog` `Observer` thread is running, the in-memory `LinkDatabase` has been fully populated from a recursive project walk, all filesystem events that arrived during the scan have been replayed, and the service is blocked in `while self.running: time.sleep(1)` awaiting real-time events (the monitor loop in `LinkWatcherService.start`). At this boundary the startup workflow ends and the real-time workflows (WF-001, WF-002, …) take over.
...
1. **`main()` — [main.py](main.py)** receives `sys.argv` via `argparse.ArgumentParser`.
...
2. **`setup_logging()` — [src/linkwatcher/logging.py](src/linkwatcher/logging.py)** receives `level`, `log_file`, and color/icon flags derived from CLI args.
...
3. **`load_config()` — [main.py](main.py)** receives `args.config` (path or None), the `argparse.Namespace`, and `project_root` string.
...
4. **`LinkWatcherService.__init__()` — [src/linkwatcher/service.py](src/linkwatcher/service.py)** receives `project_root` string and `config: LinkWatcherConfig`.
...
5. **`service.start(initial_scan=config.initial_scan_enabled)` — [src/linkwatcher/service.py](src/linkwatcher/service.py)** receives a boolean controlling whether the scan runs.
...
6. **`LinkWatcherService._initial_scan()` — [src/linkwatcher/service.py](src/linkwatcher/service.py)** receives no arguments; reads state from `self.config`, `self.project_root`, `self.parser`, `self.link_db`.
...
7. **`handler.notify_scan_complete()` — [src/linkwatcher/handler.py](src/linkwatcher/handler.py)** receives no arguments.
...
8. **Monitor loop** — the blocking `while self.running` wait at the end of `LinkWatcherService.start` ([service.py](src/linkwatcher/service.py)) is the terminal state of the startup workflow. Subsequent filesystem events (WF-001 single move, WF-002 directory move, …) are handled entirely within `LinkMaintenanceHandler` and are out of scope for this narrative.

## Callback/Event Chains

### Signal-handler registration

- **Registration**: `LinkWatcherService.__init__` ([service.py](src/linkwatcher/service.py)) calls `signal.signal(signal.SIGINT, self._signal_handler)` and the same for `SIGTERM`, guarded by the `register_signals` constructor flag (false in tests to avoid clobbering pytest's handlers).
- **Trigger**: User presses Ctrl+C, a parent sends `SIGTERM`, or the OS delivers a termination signal.
- **Handler**: `_signal_handler` ([service.py](src/linkwatcher/service.py)) sets `self.running = False`, causing the monitor loop to exit and the `finally` block to call `self.stop()` which joins the observer thread and logs final statistics.
...
- **Registration**: `self.observer.schedule(self.handler, project_root, recursive=True)` in `service.start()` ([service.py](src/linkwatcher/service.py)) installs `LinkMaintenanceHandler` as the event sink for every filesystem event beneath `project_root`.
...
|-------------|--------|-------------|-------------------|
| `initial_scan_enabled` | Config file `initial_scan_enabled` / env `LINKWATCHER_INITIAL_SCAN_ENABLED` / CLI `--no-initial-scan` | `main()` passes it to `service.start(initial_scan=...)`; consulted in `service.start` ([service.py](src/linkwatcher/service.py)) | Skips `_initial_scan()` entirely when false — observer still starts and deferral still activates, but the DB remains empty until events populate it |
| `monitored_extensions` | Config file / env (comma-separated) | `LinkMaintenanceHandler.__init__` ([handler.py](src/linkwatcher/handler.py)) AND `LinkWatcherService._initial_scan()` ([service.py](src/linkwatcher/service.py)) — both read the same `config` object | Filter applied during the walk AND during steady-state event routing; a mismatch would cause files scanned at startup to be ignored at runtime (or vice versa) |
...
| `enable_*_parser` flags (markdown, yaml, json, dart, python, powershell, generic) | Config file / env | `LinkParser.__init__` ([parser.py](src/linkwatcher/parser.py)) only — decides which specialized parser to register | Determines which file types yield references during the scan. A disabled parser means those file types fall through to the generic parser (or are skipped if generic is also disabled) |
...
| `python_source_root` | Config file / env / `doc/project-config.json` fallback | `LinkWatcherService.__init__` passes it to `LinkUpdater` only ([service.py](src/linkwatcher/service.py)) | Not consumed during startup itself — captured for later workflows so the same `LinkUpdater` can strip the src-layout prefix when updating Python imports |
...
| `dry_run_mode`, `create_backups` | Config file / env / CLI `--dry-run` | `LinkWatcherService.__init__` applies them via `self.updater.set_dry_run(...)` and `self.updater.set_backup_enabled(...)` at [service.py](src/linkwatcher/service.py) (TD235 / PD-REF-203) | Captured before `service.start()` so later workflows honor them; no effect on the scan itself (scan is read-only) |
...
- **Propagation**: `acquire_lock()` prints a red `✗ LinkWatcher is already running (PID: N)` message and calls `sys.exit(1)` directly ([main.py](main.py)). No subsequent feature is initialized. Note: the `try/except SystemExit: raise` in `main()` ([main.py](main.py)) lets this propagate without being caught by the generic `Exception` branch.
...
- **Propagation**: `main()` logs each issue at `error` level via the already-initialized logger (Feature 3.1.1), then `sys.exit(1)` ([main.py](main.py)). The `finally` block around `service.start()` is not reached because `service` is never constructed.
- **Impact**: Lock file is leaked — `release_lock()` is inside the inner `try/finally` around service construction in `main()`, past the `sys.exit(1)`. Next startup treats the lock as stale (PID no longer alive) and overrides it, so no long-term problem, but leaves a one-time warning on the next run.
...
- **Propagation**: Caught by the `try/except Exception` inside `_initial_scan()`'s per-file loop ([service.py](src/linkwatcher/service.py)). `scan_errors` is incremented; a `file_scan_failed` warning is logged with path, error message, and error type. Scan continues with the next file.
...
- **Recovery**: User sees the warning in logs and either fixes the file or invokes `service.force_rescan()` manually ([service.py](src/linkwatcher/service.py)).
...
- **Propagation**: Not caught inside `start()`; the bare `except Exception as e` at the end of `LinkWatcherService.start` ([service.py](src/linkwatcher/service.py)) logs `service_start_failed` and re-raises. The re-raised exception bubbles to `main()`'s `except Exception` branch ([main.py](main.py)), which logs `fatal_error` and `fatal_error_traceback` and calls `sys.exit(1)`. The `finally` in `main()` calls `release_lock()` before exit.
