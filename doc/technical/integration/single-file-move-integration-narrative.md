...
| `dry_run_mode` | `LinkWatcherConfig.dry_run_mode` → `LinkUpdater.dry_run` set in `LinkWatcherService.__init__` at [service.py:97](src/linkwatcher/service.py#L97) (TD235 / PD-REF-203); also runtime-mutable via `service.set_dry_run()` ([service.py:263-266](src/linkwatcher/service.py#L263-L266)) | **2.2.1** — `_write_file_safely()` short-circuit | Preview-only mode: Steps 6-9 compute replacements and log but skip the atomic write; DB mutations in Steps 7-9 still occur |
