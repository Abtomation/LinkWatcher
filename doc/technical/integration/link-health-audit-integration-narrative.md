...
> **Workflow**: WF-009 — User runs `python main.py --validate` to scan the workspace for broken file references and produces a report at `LinkWatcherBrokenLinks.txt`
...
1. **`LinkWatcherBrokenLinks.txt`** — a human-readable text report written by [LinkValidator.write_report()](src/linkwatcher/validator.py#L702) to `output_dir`. When `--log-file` (or `config.log_file`) is set, `output_dir` is that log file's parent directory; otherwise it is the project root ([main.py:304-309](main.py#L304-L309)).
...
12. **Exit — [main.py:314](main.py#L314)** receives `result.is_clean`
    ...
    - Final output: the process terminates; the user/CI inspects the exit code and/or reads `LinkWatcherBrokenLinks.txt`
...
| `--config <file>` CLI | `argparse` at [main.py:259](main.py#L259) | `load_config()` → `LinkWatcherConfig.from_file()` | Applies a YAML/JSON config as the second-precedence layer (file > env > defaults; CLI-flag overrides apply last but none target `validation_*` fields) |
| `--project-root <dir>` CLI | `argparse` at [main.py:256-258](main.py#L256-L258) | `validate_project_root()` → `LinkValidator(str(project_root), config)` | Root of the walk. Must be a valid directory; resolution is `Path(project_root).resolve()` inside the validator |
| `--log-file <path>` / `config.log_file` | `argparse` at [main.py:266](main.py#L266) / config | `main.py:304-308` determines `output_dir` for the report | Controls where `LinkWatcherBrokenLinks.txt` is written (defaults to project root when neither is set) |
| `--quiet` / `--debug` CLI | `argparse` at [main.py:265,267](main.py#L265) | `setup_logging()` at [main.py:286-290](main.py#L286-L290) | Changes log level + suppresses console output; does NOT affect the file report content or broken-link detection |
...
- **Impact**: **False positive in the report.** The `LinkWatcherBrokenLinks.txt` file grows with entries that are not actually broken. CI will exit `1` spuriously
