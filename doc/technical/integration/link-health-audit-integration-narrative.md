...
> **Workflow**: WF-009 — User runs `python main.py --validate` to scan the workspace for broken file references and produces a report at `logs/linkwatcher/LinkWatcherBrokenLinks.txt`
...
1. **`logs/linkwatcher/LinkWatcherBrokenLinks.txt`** — a human-readable text report written by [LinkValidator.write_report()](src/linkwatcher/validator.py#L702) to `output_dir`. `output_dir` is `config.validation_output_dir` (default `logs/linkwatcher`, resolved against the project root); when it is `None`, it falls back to the `--log-file`/`config.log_file` parent directory, then the project root ([main.py:406-419](main.py#L406-L419)).
...
| 0.1.3 | Configuration System | Defines the five validation-specific fields on [LinkWatcherConfig](src/linkwatcher/config/settings.py#L129-L158): `validation_extensions` (which extensions are walked), `validation_extra_ignored_dirs` (pruned subtrees), `validation_ignored_patterns` (target-substring skip list), `validation_ignore_file` (path to `.linkwatcher-ignore`), and `path_resolution_overrides` (per-folder resolution base for `/...` links; default `{}` = no-op). Also propagates the live-pipeline fields `ignored_directories` and `enable_<format>_parser` into the validator run |
...
graph TD
    ...
    Config([LinkWatcherConfig]) -->|"validation_extensions<br/>validation_extra_ignored_dirs<br/>validation_ignored_patterns<br/>path_resolution_overrides<br/>enable_*_parser"| Validator
...
12. **Exit — [main.py:314](main.py#L314)** receives `result.is_clean`
    ...
    - Final output: the process terminates; the user/CI inspects the exit code and/or reads `logs/linkwatcher/LinkWatcherBrokenLinks.txt`
...
| `--log-file <path>` / `config.log_file` | `argparse` at [main.py:293](main.py#L293) / config | `main.py:406-419` determines `output_dir` for the report | Report-location fallback **only when `validation_output_dir` is `None`** — by default `validation_output_dir` (`logs/linkwatcher`) takes precedence, so `--log-file` does not move `logs/linkwatcher/LinkWatcherBrokenLinks.txt` |
...
| `.linkwatcher-ignore` file | External file at `{project_root}/tools/linkwatcher/.linkwatcher-ignore` | `_load_ignore_file()` at validator construction | Per-file suppression rules loaded into `self._ignore_rules`. Absent file → empty rule list, silently handled |
...
- **Impact**: **False positive in the report.** The `logs/linkwatcher/LinkWatcherBrokenLinks.txt` file grows with entries that are not actually broken. CI will exit `1` spuriously
