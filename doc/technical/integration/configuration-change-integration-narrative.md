...
2. **`load_config(config_path, args, project_root)`** ([main.py:47-93](main.py#L47-L93)) assembles the precedence chain:
   - Step 2a: `config = DEFAULT_CONFIG` (module-level singleton from [defaults.py](src/linkwatcher/config/defaults.py); since PD-BUG-103 instantiated as `LinkWatcherConfig()`, so it equals the dataclass defaults).
...
|--------|-----------|--------------|-------|
| Built-in defaults | `@dataclass` field defaults; `DEFAULT_CONFIG = LinkWatcherConfig()` base instance | [settings.py](src/linkwatcher/config/settings.py), [defaults.py](src/linkwatcher/config/defaults.py) | Single source of truth since PD-BUG-103: the dataclass defaults carry the full sets (33 `monitored_extensions`, 14 `ignored_directories`) and `DEFAULT_CONFIG` is instantiated from them — guarded by `test_default_config_equals_dataclass_defaults` |
