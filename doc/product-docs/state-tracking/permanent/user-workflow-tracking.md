...
previous_location: doc/product-docs/technical/design/user-workflow-map.md
---
...
**Status derivation**: "Impl Status" is derived from [feature-tracking.md](/doc/product-docs/state-tracking/permanent/feature-tracking.md). "E2E Status" is derived from [e2e-test-tracking.md](/test/state-tracking/permanent/e2e-test-tracking.md). Both will be auto-updated by `Update-WorkflowTracking.ps1` (called by `Update-FeatureImplementationState.ps1` and `Update-TestExecutionStatus.ps1`) once implemented.

## Workflows

| ID | Workflow | User Action | Required Features | Priority | Impl Status | E2E Status |
|----|----------|-------------|-------------------|----------|-------------|------------|
| WF-001 | Single file move → links updated | Move/rename a file (VS Code, File Explorer, git) | 1.1.1, 2.1.1, 2.2.1 | P1 | All Implemented | ✅ Covered (TE-E2G-001–004) |
| WF-002 | Directory move → contained refs updated | Move/rename a directory | 1.1.1, 0.1.2, 2.1.1, 2.2.1 | P1 | All Implemented | ✅ Covered (TE-E2G-005) |
| WF-003 | Startup → initial project scan | Run `python main.py` or startup script | 0.1.1, 0.1.3, 0.1.2, 2.1.1, 1.1.1, 3.1.1 | P1 | All Implemented | 🔴 Partial (PD-BUG-053) |
| WF-004 | Rapid sequential moves → consistency | Move multiple files in quick succession | 1.1.1, 0.1.2, 2.2.1 | P2 | All Implemented | ✅ Covered (TE-E2G-007) |
| WF-005 | Multi-format file move → all parsers handle | Move a file referenced from MD, YAML, JSON, Python, PS1 | 2.1.1, 2.2.1, 1.1.1 | P2 | All Implemented | ✅ Covered (TE-E2G-008) |
...
| WF-007 | Dry-run mode → preview without changes | Start with `--dry-run`, move files, observe logs | 0.1.3, 0.1.1, 2.2.1, 3.1.1 | P3 | All Implemented | ✅ Covered (TE-E2G-009) |
| WF-008 | Graceful shutdown → no corrupted files | Stop the LinkWatcher process (Ctrl+C, kill) | 0.1.1, 2.2.1, 0.1.2 | P2 | All Implemented | ✅ Covered (TE-E2G-010) |
