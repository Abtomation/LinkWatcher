...
> **📋 Primary Documentation**: [TDD PD-TDD-022](../../technical/architecture/design-docs/tdd/tdd-0-1-2-in-memory-database-t2.md)
>
...
- **0.1.2-FR-1**: The system SHALL store all discovered `LinkReference` instances in an in-memory database indexed by their target file path, enabling instant reverse lookup
- **0.1.2-FR-2**: The system SHALL provide O(1) lookup time for finding all files that reference a given target file path
- **0.1.2-FR-3**: The system SHALL handle concurrent access safely when the watchdog observer thread and main service thread access the database simultaneously
- **0.1.2-FR-4**: The system SHALL update all stored references when a file is moved, changing the target path in all affected `LinkReference` instances
- **0.1.2-FR-5**: The system SHALL remove all references from a source file when that file is deleted or its content is rescanned
- **0.1.2-FR-6**: The system SHALL normalize paths consistently to ensure lookups succeed regardless of how the path was originally stored
- **0.1.2-FR-7**: The system SHALL provide database statistics (total links count, total unique targets) for operational reporting

### User Interactions

- **0.1.2-UI-1**: Users do not interact directly with the database — it operates transparently in the background
- **0.1.2-UI-2**: Users see the effect of the database through instant link updates when files are moved (sub-second response time enabled by O(1) lookups)
- **0.1.2-UI-3**: Users receive database statistics on shutdown showing how many links were tracked: "Total links in database: 1,234 | Total unique targets: 567"
- **0.1.2-UI-4**: Users experience reliable operation even in concurrent scenarios (file scanning while new file events arrive) due to thread-safe implementation

### Business Rules

- **0.1.2-BR-1**: Link storage is indexed by target path (not source path), optimizing for the critical operation: "what files reference this moved file?"
...
- [x] **0.1.2-AC-1**: Database provides O(1) lookup time for target path queries (confirmed via unit tests with 10,000+ links)
- [x] **0.1.2-AC-2**: Concurrent access from multiple threads does not corrupt data (tested with simultaneous add/query operations)
- [x] **0.1.2-AC-3**: Moving a file triggers `update_target_path()` which correctly updates all references in the database
- [x] **0.1.2-AC-4**: Path lookups succeed for links stored with anchors (e.g., `file.md#section`) when queried without anchors
- [x] **0.1.2-AC-5**: Path lookups succeed for relative paths that are queried as absolute paths (and vice versa)
- [x] **0.1.2-AC-6**: Database statistics accurately reflect the count of total links and unique target files
- [x] **0.1.2-AC-7**: Data is cleared when `clear()` is called, resetting the database to empty state
...
- **0.1.2-EC-1**: If a path is queried that has no references, `get_references_to_file()` returns an empty list (not an error)
- **0.1.2-EC-2**: If `update_target_path()` is called for a target with no references, the operation completes silently (no error, no effect)
- **0.1.2-EC-3**: If the same `LinkReference` is added multiple times (duplicate calls), each is stored separately as the database does not enforce uniqueness
- **0.1.2-EC-4**: If concurrent threads access the database simultaneously, the single `threading.Lock` serializes operations, preventing race conditions
- **0.1.2-EC-5**: If path normalization fails (malformed path), the path is stored as-is and may not match during lookups — this is handled by the three-level resolution strategy
- **0.1.2-EC-6**: If the database grows very large (tens of thousands of links), lookup remains O(1) but memory usage increases linearly (in-memory only, no paging)
...
- [x] All functional requirements clearly defined with Feature ID prefixes (0.1.2-FR-1 through 0.1.2-FR-7)
...
- [x] Acceptance criteria are testable and measurable (0.1.2-AC-1 through 0.1.2-AC-7)
- [x] Edge cases identified with expected behaviors (0.1.2-EC-1 through 0.1.2-EC-6)
