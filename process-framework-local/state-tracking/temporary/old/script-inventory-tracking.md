...
1. **Use Relative Paths**: Scripts use paths relative to project root (e.g., `process-framework/templates/...`)
...
echo Set-Location 'c:\Users\ronny\VS_Code\LinkWatcher'; ^& process-framework/scripts/file-creation/SCRIPT-NAME.ps1 -Params > temp.ps1 && pwsh.exe -ExecutionPolicy Bypass -File temp.ps1 && del temp.ps1
```
...
```powershell
Move-Item "process-framework-local/state-tracking/temporary/script-inventory-tracking.md" `
          "process-framework-local/state-tracking/temporary/old/script-inventory-tracking.md"
```
...
- **This Tracking File**: `process-framework-local/state-tracking/temporary/script-inventory-tracking.md`
