...
| Source | [PF-IMP-258](/process-framework-local/state-tracking/permanent/process-improvement-tracking.md) |
...
`user-workflow-tracking.md` (PD-DES-002) currently lives in `doc/technical/design` as a design artifact. It is treated as a one-time planning output, not as a living state file.

### Proposed Change

**Promote `user-workflow-tracking.md` to a state tracking artifact** by:

1. **Relocating** from `doc/technical/design` to `doc/state-tracking/permanent`
...
|---------------------|-----------------|------------------|-----------|
| **bug-tracking.md** | `doc/state-tracking/permanent` | **YES — HIGH** | Bugs impact workflows differently. A bug in 2.2.1 affects 6/8 workflows. Workflow column enables workflow-scoped triage, blast radius assessment, and regression scope. |
| **technical-debt-tracking.md** | `doc/state-tracking/permanent` | **YES — HIGH** | Tech debt affects specific workflows. Knowing WF-IDs enables workflow-scoped debt analysis ("what debt blocks WF-001?") and prioritization by workflow criticality. Refactoring tasks (PF-TSK-007) benefit from knowing which workflows are impacted by the debt being resolved. |
| **Feature state files** | `doc/state-tracking/features` | **YES — MEDIUM** | Add "Workflows" metadata field (list of WF-IDs). Enables implementation and validation tasks to see workflow context without looking up the workflow map separately. |
...
|----------|------|-----|----------|
| `user-workflow-tracking.md` | `doc/technical/design` | `doc/state-tracking/permanent` | `user-workflow-tracking.md` |
...
- [ ] Relocate `user-workflow-tracking.md` to `doc/state-tracking/permanent/user-workflow-tracking.md`
...
- [ ] Final PF-documentation-map.md sweep
