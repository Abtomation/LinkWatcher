...
| PF-VAL | Process Framework | Product validation | Validation reports live under `doc/validation` |
| PF-REF | Process Framework | Product refactoring | Refactoring plans live under `doc/refactoring` |
| PF-TDA/TDI/TDM | Process Framework | Product tech debt | Tech debt items are project-specific |
| ART-ASS | Artifact | Product assessments | Assessment artifacts live under `doc/` |
| ART-FEE | Artifact | Process framework | Feedback forms live under `process-framework/feedback` |
| ART-REV | Artifact | Process framework | Review artifacts live under `process-framework/feedback` |
| PD-GDE | Product Documentation | Process framework | Points to `process-framework/guides` — wrong domain |
...
|---|---|---|
| **PD-ASS** (from ART-ASS) | PD-ASS (Assets, nextAvailable: 3) | **No collision**: Existing PD-ASS is orphaned — 0 files found, `doc/technical/assets` directory deleted. Simply replace the prefix entry with ART-ASS data, keeping ART-ASS counter at 201. |
| **PF-FEE** (from ART-FEE) | PF-FEE (Feedback, nextAvailable: 2) | **No collision**: The 1 existing PF-FEE file is the feedback README (`process-framework/feedback/README.md`) — this file will be removed. ART-FEE takes over PF-FEE cleanly, keeping counter at 435. |

#### Missing Metadata

| Prefix | Files | Issue | Action |
|---|---|---|---|
| PD-UGD | `doc/user/handbooks/file-type-quick-fix.md`, `doc/user/handbooks/troubleshooting-file-types.md` | No YAML frontmatter metadata | Add `id: PD-UGD-001` / `PD-UGD-002` metadata to each file |

### Final Prefix Assignment

**PF-id-registry.json** (`process-framework`) — Process Framework:
...
**PD-id-registry.json** (`doc/`) — Product Documentation:
...
function Get-IdRegistryPath {
    ...
    $registryMap = @{
        ...
        'PD-' = Join-Path $docDir "doc/PD-id-registry.json"
...
- **Framework portability**: The `process-framework` directory becomes self-contained — it can be copied to another project without carrying project-specific product IDs and counters
...
| File | Change |
|------|--------|
| `process-framework/scripts/IdRegistry.psm1` | Add hardcoded prefix→registry mapping to `Get-IdRegistryPath` and propagate prefix through all functions |

### Validation Scripts (must change)

| File | Change |
|------|--------|
| `process-framework/scripts/validation/Validate-IdRegistry.ps1` | Scan all three registry files |
| `process-framework/scripts/validation/Validate-StateTracking.ps1` | Update hardcoded `doc/id-registry.json` path |
| `process-framework/scripts/validation/Validate-TestTracking.ps1` | Update hardcoded `doc/id-registry.json` path |

### File Creation Scripts (must change)

| File | Change |
|------|--------|
| All scripts in `process-framework/scripts/file-creation` | Update to pass prefix through to `Get-IdRegistryPath` or update hardcoded paths |
| `process-framework/scripts/file-creation/03-testing/New-TestSpecification.ps1` | Also update prefix from PF-TSP to TE-TSP |
| `process-framework/scripts/file-creation/03-testing/New-E2EAcceptanceTestCase.ps1` | Already uses TE-* prefix — update path only |
| `process-framework/scripts/file-creation/05-validation/New-ValidationReport.ps1` | Also update prefix from PF-VAL to PD-VAL |
...
| `process-framework/PF-documentation-map.md` | Update all references to renamed prefixes (PF-VAL-035 → PD-VAL-035, PF-TSP-035 → TE-TSP-035, etc.) |
...
| File | Action |
|------|--------|
| `process-framework/feedback/README.md` | Remove (PF-FEE-000) — eliminates PF-FEE collision |

### Files to Move

| File | From | To | Reason |
|------|------|----|--------|
| `doc/id-registry.json` | `doc/` | `process-framework/PF-id-registry.json` | Becomes PF registry; LinkWatcher updates references |
| `process-framework-local/state-tracking/permanent/test-tracking.md` | `process-framework` | `test/test-tracking.md` | Test state tracking belongs in test directory |

## Migration Strategy

### Phase 1: Create Registry Files by Moving and Splitting

1. **Move** `doc/id-registry.json` → `process-framework/PF-id-registry.json` (LinkWatcher updates references automatically)
...
3. **Create** `doc/PD-id-registry.json` with PD-* prefixes (including renames: PF-STA product files → PD-STA, PF-VAL → PD-VAL, PF-REF → PD-REF, PF-TDA/TDI/TDM → PD-*, ART-ASS → PD-ASS, PF-FEA → PD-FIS; remove PD-GDE, PD-USR, orphaned old PD-ASS)
...
5. Remove feedback README (`process-framework/feedback/README.md`) — eliminates PF-FEE collision
...
2. Update PF-documentation-map.md with renamed prefix IDs
...
- `process-framework` is self-contained for framework IDs (can be copied to another project)
...
- Framework portability: `process-framework` can be copied to a new project and `IdRegistry.psm1` functions work for PF-* prefixes without modification
