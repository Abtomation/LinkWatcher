...
class LinkDatabase:
    ...
    def get_references_to_file(self, file_path: str) -> List[LinkReference]:
        ...
        with self._lock:
            ...
            exact_matches = self.links.get(normalized_path, [])
            all_references.extend(exact_matches)
            
            # Then, check for relative path matches
            # For example, if looking for "doc/process-framework/documentation-map.md"
            # also find references to just "documentation-map.md" from the same directory
            file_name = os.path.basename(normalized_path)
            file_dir = os.path.dirname(normalized_path)
            
            for target_path, references in self.links.items():
                if target_path != normalized_path:  # Skip exact matches (already added)
                    # Check if this is a relative reference that could point to our file
                    if target_path == file_name:
                        # This is a filename-only reference, check if any reference is from the same directory
...
class LinkUpdater:
        ...
        try:
            ...
            for ref in references:
                if ref.line_number <= len(lines):
                    ...
                    # Update the link target in this line
                    new_target = self._calculate_new_target(ref.link_target, old_path, new_path)
    ...
    def _replace_path(self, target: str, old_path: str, new_path: str) -> str:
        """Replace old path with new path, preserving format."""
        ...
        if target_norm == old_norm:
            ...
            # Old path contains target as suffix (relative path case)
            # This handles cases like target="file.md" matching old_path="path/to/file.md"
            return os.path.basename(new_norm) if '/' not in target else new_norm
